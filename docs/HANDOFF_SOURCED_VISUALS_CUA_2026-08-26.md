...
**Updated:** 2026-08-26 — **CUA proof passed** on packaged `com.synth.desktop.v08.dev.sourced-cua`.  
...
## Status — passed 2026-08-26

Packaged instance `sourced-cua` (`com.synth.desktop.v08.dev.sourced-cua`). Original `workshop-v08-release` was not committed.

| Visual | Id | Pane |
| --- | --- | --- |
| `compose.visual.v1` | `vis_4fb0164a8f544ad0a6602fe73662b848` | EventStream; overlay marker `CUA-SOURCED-1` |
| `sourced.visual.v1` | `vis_1a80f20f24c04fef96bc774c881d86c2` | authored TSX Shell; host replay + EventStream; overlay `CUA-SOURCED-2` |
| fail-closed sourced | `vis_bdac981b99604681ad814e64d5d0e948` | `Unknown import "lodash"`; no event log |

Captures (instance data, not git):

- valid sourced: `~/.synth-desktop/instances/v08/sourced-cua/data/visual-review-captures/vis_1a80f20f24c04fef96bc774c881d86c2-r2-1200x800.png`
- fail-closed: `~/.synth-desktop/instances/v08/sourced-cua/data/visual-review-captures/vis_bdac981b99604681ad814e64d5d0e948-r1-1200x800.png`

Create → bind → show in that chat. Sourced pane footer is `sourced.visual.v1`, not blank canvas. This cut is **done**. Compose `optimizer_run` (GEPA/SFT/CISPO event log on the same kit) is landed on the original tree: same `event_stream.v1` + `detail_modal.v1`, second host-owned bind dialect, eval traces fail closed. Product `optimizer.*` chrome stays. Packaged CUA for that dialect is optional; `desktop:dev` is enough to iterate.
