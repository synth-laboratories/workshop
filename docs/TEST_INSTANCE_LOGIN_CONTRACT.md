...
instances. The canonical `~/.codex/auth.json` is a read-only seed; refreshes,
re-authentication, and disconnect state are persisted at
`~/.synth-desktop/shared/oauth/codex.json` with mode `0600`. A cross-process
lock serializes token rotation, so every named instance reuses the latest valid
credential without sharing its histories, databases, workspace, or Codex home.
Testing instances must not create, read, or modify a macOS Keychain credential.
...
5. A newly named instance reuses the shared ChatGPT session and requires a
   browser flow only when refresh is genuinely rejected or the user disconnects.
6. `./scripts/test-desktop-instance.sh` passes.
