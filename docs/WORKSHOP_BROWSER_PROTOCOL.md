...
- Browser approval cards render bounded nested action metadata (origin, target, tab/revision, value length/digest, file paths, dialog choice) instead of JavaScript object placeholders. The projection regression proves field contents are absent while the exact-action metadata remains human-readable.
...
- `scripts/browser-update-roundtrip.sh` is the non-notary release rehearsal: it performs two separate Tauri builds with distinct versions, uses the same staged bundle replacement/backup shape as the desktop installer in an isolated temporary Applications directory, launches and drives the installed app through authenticated IPC before the update, after the update, and after rollback, restores the exact backup, and opens one persistent browser profile in all three states. This is deliberately called bundle replacement rather than an in-app updater: v0.5 still has only a passive update check and fixed download page.
- `scripts/browser-soak.mjs` runs the reference backend in headed mode for 30 minutes by default, continuously mutates and reads a local SPA, churns tabs, enforces a 2,000-character observation ceiling, samples the Chromium process tree, and fails on any backend exit or GPU-process restart. `verify-browser-production-gates.sh soak APP [SECONDS]` writes the machine-readable receipt.
- `scripts/browser-live-approval-e2e.mjs` drives a real named Workshop session and normal browser MCP call to a pending consequential action. The acceptance operator must use macOS UI automation to click the rendered **Approve once** control; the harness then proves the exact prepared action executed and was audited exactly once. It does not expose an agent-callable approval resolver or a test-only production command.
...
- Developer ID/notarization, a future true in-app updater artifact pair, and the CEF Workshop embedding receipt remain external evidence gates. The isolated two-build staged replacement/rollback, long headed soak, and live approval-card click are non-notary v0.5 gates; none substitutes for Apple release acceptance or CEF embedding evidence.
