...
- `scripts/browser-update-roundtrip.sh` is the non-notary release rehearsal: it performs two separate Tauri builds with distinct versions, uses the same staged bundle replacement/backup shape as the desktop installer in an isolated temporary Applications directory, restores the exact backup, and opens one persistent browser profile before the update, after the update, and after rollback. This is deliberately called bundle replacement rather than an in-app updater: v0.5 still has only a passive update check and fixed download page.
