...
test("v0.1 hides Synth Cloud Laguna S even when an api key is configured", async ({ page }) => {
	...
	await expect(menu.getByTestId("composer-model-option-synth-cloud-laguna-s")).toHaveCount(0);
...
test("Synth Cloud Laguna S remains hidden when api key is missing", async ({ page }) => {
	// Default browser stub has apiKeyConfigured: false — do not override it.
	await page.getByTestId("composer-model").click();
	await expect(page.getByTestId("composer-model-option-synth-cloud-laguna-s")).toHaveCount(0);
});
