...
test("a failed consent save keeps the question available for retry", async ({ page }) => {
	await page.evaluate(() => {
		const telemetry = window.synthTelemetry!;
		const original = telemetry.setConsent;
		telemetry.setConsent = async (granted) => {
			telemetry.setConsent = original;
			throw new Error("simulated persistence failure");
		};
	});
	const ask = page.getByTestId("telemetry-consent-ask");
	await ask.getByTestId("telemetry-consent-decline").click();
	await expect(ask.getByRole("alert")).toContainText("Could not save");
	await expect(ask).toBeVisible();
	await ask.getByTestId("telemetry-consent-decline").click();
	await expect(ask).not.toBeVisible();
});

test("a failed event read is not presented as an empty outbox", async ({ page }) => {
	await page.evaluate(() => {
		window.synthTelemetry!.recent = async () => { throw new Error("simulated read failure"); };
	});
	await openSettings(page);
	await page.getByTestId("telemetry-view-events").click();
	await expect(page.getByRole("alert")).toContainText("Could not load collected events");
	await expect(page.getByTestId("telemetry-event-list")).not.toBeVisible();
});
