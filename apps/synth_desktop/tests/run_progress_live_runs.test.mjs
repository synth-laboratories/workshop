...
test("a real GEPA run reaches a usable estimate mid-flight, and warms first", () => {
	// The first frames have too little evidence; later ones do not.
	const early = replay(CAPTURE.gepa, 12);
	assert.ok(
		early.every((frame) => ["estimating", "unavailable", "point", "range"].includes(frame.timing.eta.state)),
		"every early frame produces a renderable ETA state"
	);
	assert.ok(
		early[0].timing.eta.state === "estimating" || early[0].timing.eta.state === "unavailable",
		`the first frame cannot estimate, got ${early[0].timing.eta.state}`
	);

	const midpoint = Math.floor(CAPTURE.gepa.events.length / 2);
	const mid = projectRunProgress(
		snapshot(CAPTURE.gepa, { events: CAPTURE.gepa.events.slice(0, midpoint), status: "running" }),
		NOW
	);
	assert.ok(
		mid.timing.eta.state === "point" || mid.timing.eta.state === "range",
		`a half-replayed real run should offer an estimate, got ${mid.timing.eta.state}: ${mid.timing.eta.basis}`
	);
	assert.match(mid.timing.eta.basis, /phase /);
	assert.match(formatEta(mid.timing.eta), /remaining$/);
});
