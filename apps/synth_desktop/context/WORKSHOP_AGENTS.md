...
- Do not read `.env`, `.env.*`, `secrets.toml`, or other plaintext credential
  ...
  has no read-denylist field — this file and the installed task skill are the
  policy. Use only an already-authorized project-local credential source and
  Workshop's ephemeral secrets proxy. Never use the Keychain-backed Secrets
  registry or its credential-import flow, and never access macOS Keychain.
