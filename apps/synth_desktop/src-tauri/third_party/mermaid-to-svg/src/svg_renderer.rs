...
const EDGE_LABEL_PADDING_H: f64 = 8.0;
const EDGE_LABEL_PADDING_V: f64 = 4.0;
...
"#,
  ...
  <filter id="soft-shadow" x="-20%" y="-20%" width="140%" height="150%">
    <feDropShadow dx="0" dy="2" stdDeviation="2.5" flood-color="#101828" flood-opacity="0.12"/>
  </filter>
...
"##,
    ...
    fn render_rectangle(&mut self, node: &LayoutNode, rx: f64) {
        ...
        let state_polish = if self.is_state_diagram {
            r#" filter="url(#soft-shadow)""#
        } else {
            ""
        };
        let radius = if self.is_state_diagram { 10.0 } else { rx };
        let stroke_width = if self.is_state_diagram { 1.5 } else { 1.0 };
        self.output.push_str(&format!(
            r#"<rect x="{:.1}" y="{:.1}" width="{:.1}" height="{:.1}" rx="{:.1}" fill="{}" stroke="{}" stroke-width="{:.1}"{}/>
"#,
            x, y, node.width, node.height, radius, fill, stroke, stroke_width, state_polish
        ));
...
"#,
    ...
    fn render_edge_labels(&mut self, edges: &[LayoutEdge]) {
        ...
        for info in &labels {
            ...
            self.output.push_str(&format!(
                r##"<rect x="{:.1}" y="{:.1}" width="{:.1}" height="{:.1}" fill="#ffffff" stroke="#d0d5dd" stroke-width="1" rx="8"/>
"##,
                rect_x, rect_y, info.width, info.height
            ));
