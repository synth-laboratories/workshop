...
const STATE_NODE_WIDTH_PADDING: f64 = 28.0;
const STATE_NODE_HEIGHT_PADDING: f64 = 14.0;
const STATE_NODE_MIN_HEIGHT: f64 = 42.0;
...
impl<'a> LayoutEngine<'a> {
    ...
    fn compute_spacing(&self) -> (f64, f64) {
        if self.is_state_diagram {
            (
                self.options.node_spacing.max(72.0),
                self.options.rank_spacing.max(66.0),
            )
        } else {
