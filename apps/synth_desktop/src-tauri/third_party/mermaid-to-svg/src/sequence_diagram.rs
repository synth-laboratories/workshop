                ...
                if (x1 - x2).abs() < 1.0 {
                    ...
                    if !text.is_empty() {
                        svg.push_str(&format!(
                            ...
                            theme.text_color,
                            render_sequence_text(text, x1 + 4.0, "start"),
                            tx = x1 + 4.0,
                            ty = y - 6.0
                        ));
                    }
                    ...
                    if !text.is_empty() {
                        ...
                        svg.push_str(&format!(
                            ...
                            theme.text_color,
                            render_sequence_text(text, mx, "middle"),
                            ty = y - 6.0
                        ));
                    }
                }
...
fn estimate_sequence_text_width(text: &str) -> f64 {
    text.split("<br/>")
        .map(|line| crate::text_wrap::display_width_units(line) * 6.5)
        .fold(0.0_f64, f64::max)
}

fn render_sequence_text(text: &str, x: f64, anchor: &str) -> String {
    ...
    for (index, line) in lines.iter().enumerate() {
        let dy = if index == 0 { 0.0 } else { 14.0 };
        out.push_str(&format!(
            "<tspan x=\"{x:.3}\" dy=\"{dy:.3}\" text-anchor=\"{anchor}\">{}</tspan>",
            escape_xml(line)
        ));
    }
    out
