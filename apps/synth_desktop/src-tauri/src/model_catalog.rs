...
mod tests {
    ...
    #[test]
    fn observed_metadata_only_admits_provider_claimed_capabilities() {
        let model: OpenRouterApiModel = serde_json::from_value(serde_json::json!({
            "id": "stealth/ox-alpha",
            "name": "Ox Alpha",
            "context_length": 1048576,
            "top_provider": { "max_completion_tokens": 131072 },
            "architecture": { "input_modalities": ["text", "image", "video"], "output_modalities": ["text"] },
            "supported_parameters": ["tools", "reasoning", "reasoning_effort", "include_reasoning"],
            "expiration_date": "2098-12-31"
        }))
        .unwrap();
        let observed = ObservedModel::from_api(&model, "2026-08-25T00:00:00Z");
        assert_eq!(observed.display_name, "Ox Alpha");
        assert!(observed.capabilities.tools);
        assert_eq!(
            observed.capabilities.reasoning_control,
            ModelCatalogReasoningControl::Effort
        );
        assert_eq!(observed.capabilities.max_context_tokens, Some(1_048_576.0));
        assert_eq!(observed.capabilities.max_completion_tokens, Some(131_072.0));
        assert_eq!(observed.expires_at.as_deref(), Some("2098-12-31"));
        assert_eq!(
            parse_timestamp(observed.expires_at.as_deref().unwrap()),
            Some(
    ...
    #[test]
    fn ui_override_can_only_narrow_reasoning() {
        let mut capabilities = ModelCatalogCapabilities {
            reasoning_control: ModelCatalogReasoningControl::Binary,
            default_reasoning: Some("max".into()),
            ..Default::default()
        };
        narrow_reasoning_capability(
            &mut capabilities,
            Some(&OpenRouterModelUiConfig {
                reasoning_control: Some(OpenRouterReasoningControl::Effort),
        ...
        assert_eq!(capabilities.default_reasoning, None);
    }

    #[test]
    fn configured_model_keeps_its_exact_slug_and_stable_target_id() {
        let candidates = candidates(&[OpenRouterModelConfig {
            id: "ox-alpha".into(),
        ...
        let target = candidates
            .iter()
            .find(|candidate| candidate.target_id == "openrouter:ox-alpha")
            .expect("configured entry");
        assert_eq!(target.model_id, "stealth/ox-alpha");
        assert_ne!(target.model_id, "poolside/laguna-s-2.1");
    }
}
