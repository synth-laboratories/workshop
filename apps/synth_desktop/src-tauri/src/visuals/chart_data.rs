...
mod tests {
    ...
    fn rows(value: Value) -> Vec<Row> {
        rows_from(&value).expect("rows")
    }

    fn source(transform: Value) -> DataSource {
    ...
    fn document() -> Value {
        json!({"steps":[
            {"turn":0,"action":"noop","reward":0.0,"achievements":[],"metrics":{"health":9}},
            {"turn":1,"action":"move_left","reward":0.1,"achievements":["drink"],"metrics":{"health":9}},
            {"turn":2,"action":"do","reward":1.0,"achievements":["drink","wood"],"metrics":{}},
            {"turn":3,"action":"do","achievements":[],"metrics":{"health":7}}
        ]})
    }

    /// Developer utility, like `charts::dump_reference_chart`, but for a spec
    /// whose panels are derived:
    ///
    ///   cargo test -p synth-desktop --lib dump_derived_chart -- --ignored
    #[test]
    #[ignore]
    fn dump_derived_chart() {
        let root = crate::visuals::templates_root_for_tests();
        let document: Value = serde_json::from_slice(
            &std::fs::read(root.join("fixtures/rollout_steps.json")).expect("fixture"),
        )
        .expect("fixture json");
        let spec: super::super::charts::ChartSpec =
            serde_json::from_str(DERIVED_SPEC).expect("spec");
        let mut documents = BTreeMap::new();
        documents.insert("rollout".to_string(), document);
        let resolved = resolve(&spec, &documents).expect("resolve");
        let rendered = super::super::charts::render_spec(&resolved).expect("render");
        std::fs::write("target/chart-derived.svg", rendered.svg).expect("write");
    }

    const DERIVED_SPEC: &str = r#"{
      "version": 1,
      "title": "Craftax rollout",
      "subtitle": "derived from fixtures/rollout_steps.json — no values typed into the spec",
      "panels": [
        {"kind":"metrics","from":{
          "source":{"slot":"rollout","path":"steps","transform":[
            {"op":"groupAggregate","aggregate":{
              ...
              "peak_health":{"func":"max","field":"metrics.health"},
              "unmeasured":{"func":"mean","field":"metrics.mana"}}}]},
          "items":[{"label":"turns","value":"turns"},
                   {"label":"total reward","value":"reward"},
                   {"label":"peak health","value":"peak_health"},
                   {"label":"mana","value":"unmeasured"}]}},
        {"kind":"series","title":"Cumulative reward","y":{"label":"reward"},"x":{"label":"turn"},"from":{
          "source":{"slot":"rollout","path":"steps","transform":[
            {"op":"sort","by":"turn"},
            {"op":"derive","field":"total","from":{"cumulative":"reward"}}]},
          "series":[{"name":"cumulative","x":"turn","y":"total","style":"area"},
                    {"name":"per turn","x":"turn","y":"reward"}]}},
        {"kind":"bars","title":"Actions taken","orientation":"horizontal","y":{"label":"steps"},"from":{
          "source":{"slot":"rollout","path":"steps","transform":[
        ...
        {"kind":"heatmap","title":"Vitals by turn","unit":"","from":{
          "source":{"slot":"rollout","path":"steps","transform":[
            {"op":"unpivot","fields":["metrics.health","metrics.food","metrics.drink","metrics.energy"],
             "nameAs":"vital","valueAs":"level"}]},
          "row":"vital","column":"turn","value":"level"}},
        {"kind":"table","title":"First unlock","from":{
          "source":{"slot":"rollout","path":"steps","transform":[
            {"op":"unwind","field":"achievements","as":"achievement"},
            {"op":"groupAggregate","by":["achievement"],
             "aggregate":{"first_turn":{"func":"min","field":"turn"},"steps":{"func":"count"}}},
            {"op":"sort","by":"first_turn"}]},
          "columns":[{"header":"achievement","field":"achievement"},
                     {"header":"first turn","field":"first_turn"},
                     {"header":"steps seen","field":"steps"}]}}
      ]
    }"#;

    #[test]
    fn paths_reach_into_nested_evidence() {
        let table = rows(json!([{"metrics":{"health":9}},{"metrics":{}}]));
        assert_eq!(numeric_field(&table[0], "metrics.health"), Some(9.0));
        assert_eq!(numeric_field(&table[1], "metrics.health"), None);
    }

    #[test]
    fn a_scalar_array_becomes_a_value_column() {
        let table = rows(json!([1, 2, 3]));
        assert_eq!(table.len(), 3);
        assert_eq!(numeric_field(&table[1], "value"), Some(2.0));
    }

    #[test]
    fn an_absent_measurement_is_never_treated_as_zero() {
        // The fourth step has no reward at all.
        let derived = table(&document(), &source(json!([]))).expect("table");
        assert_eq!(numeric_field(&derived[3], "reward"), None);

        // A numeric filter must not admit it as 0.
        let filtered = table(
            &document(),
            &source(json!([{"op":"filter","field":"reward","is":{"lte":0.0}}])),
        )
        .expect("filter");
        assert_eq!(filtered.len(), 1, "only the real zero survives");
        assert_eq!(numeric_field(&filtered[0], "turn"), Some(0.0));

        // And an aggregate over nothing is absent, not zero.
        let empty = table(
            &document(),
            &source(json!([
                {"op":"filter","field":"action","is":{"eq":"never"}},
                {"op":"groupAggregate","aggregate":{"mean":{"func":"mean","field":"reward"},"rows":{"func":"count"}}}
            ])),
        )
        .expect("aggregate");
        assert_eq!(empty.len(), 1);
        assert_eq!(empty[0]["mean"], Value::Null);
        assert_eq!(
            empty[0]["rows"],
            json!(0.0),
            "a count is defined over zero rows"
        );
    }

    #[test]
    fn group_aggregate_keeps_first_appearance_order() {
        let derived = table(
            &document(),
            &source(json!([
                {"op":"groupAggregate","by":["action"],
                 "aggregate":{"steps":{"func":"count"},"reward":{"func":"sum","field":"reward"}}}
            ])),
        )
        .expect("group");
        let actions: Vec<&str> = derived
            .iter()
            .map(|row| row["action"].as_str().unwrap_or_default())
            .collect();
        assert_eq!(actions, vec!["noop", "move_left", "do"]);
        assert_eq!(derived[2]["steps"], json!(2.0));
        assert_eq!(derived[2]["reward"], json!(1.0));
    }

    #[test]
    fn unwind_explodes_a_nested_list_and_can_keep_the_empties() {
        let dropped = table(
            &document(),
            &source(json!([{"op":"unwind","field":"achievements","as":"achievement"}])),
        )
        .expect("unwind");
        assert_eq!(dropped.len(), 3);
        assert_eq!(dropped[0]["achievement"], json!("drink"));

        let kept = table(
            &document(),
            &source(
                json!([{"op":"unwind","field":"achievements","as":"achievement","keepEmpty":true}]),
            ),
        )
        .expect("unwind");
        assert_eq!(kept.len(), 5);
        assert!(kept.iter().any(|row| row["achievement"].is_null()));
    }

    #[test]
    fn cumulative_carries_across_a_gap_without_inventing_a_value() {
        let derived = table(
            &document(),
            &source(json!([{"op":"derive","field":"total","from":{"cumulative":"reward"}}])),
        )
        .expect("derive");
        assert_eq!(derived[2]["total"], json!(1.1));
        // The gap keeps the running total rather than resetting it or adding 0.
        assert_eq!(derived[3]["total"], json!(1.1));
    }

    #[test]
    fn delta_and_ratio_refuse_to_guess() {
        let derived = table(
            &document(),
            &source(json!([
                {"op":"derive","field":"step_delta","from":{"delta":"reward"}},
                {"op":"derive","field":"per_health","from":{"ratio":["reward","metrics.health"]}}
            ])),
        )
        .expect("derive");
        assert_eq!(derived[0]["step_delta"], Value::Null, "no previous row");
        assert_eq!(derived[1]["step_delta"], json!(0.1));
        assert_eq!(derived[2]["per_health"], Value::Null, "no health measured");
        assert_eq!(derived[3]["per_health"], Value::Null, "no reward measured");
    }

    #[test]
    fn percentiles_interpolate_like_the_published_page() {
    ...
    #[test]
    fn a_missing_path_names_what_the_document_actually_has() {
        let error = table(
            &document(),
            &serde_json::from_value::<DataSource>(json!({"slot":"rollout","path":"rollouts"}))
                .unwrap(),
        )
        .expect_err("missing path must refuse");
        let message = format!("{error:#}");
        assert!(message.contains("rollouts"), "{message}");
        assert!(
    ...
    #[test]
    fn unknown_transform_fields_are_refused_rather_than_ignored() {
        let bad: Result<DataSource, _> = serde_json::from_value(json!({
            "slot":"rollout",
            "transform":[{"op":"sort","by":"turn","descending":true}]
        }));
        assert!(bad.is_err(), "a misspelled option must not be dropped");
    }
}
