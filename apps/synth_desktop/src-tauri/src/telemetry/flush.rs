...
mod tests {
    ...
    #[tokio::test]
    async fn events_sync_after_withdrawal_clears_the_outbox() {
        let (_dir, store, sink, flusher) = fixture();
        consent::record_choice(&store, consent::ConsentChoice::Granted).unwrap();
        record(&store, "signin_completed");
        assert_eq!(
        ...
        let previous = store.watermark().unwrap();
        consent::record_choice(&store, consent::ConsentChoice::Declined).unwrap();
        store.delete_optional().unwrap();
        consent::record_choice(&store, consent::ConsentChoice::Granted).unwrap();
        record(&store, "signin_completed");
        assert_eq!(
            flusher.flush_once().await.unwrap(),
        ...
        assert!(store.watermark().unwrap() > previous);
        assert_eq!(sink.batches.lock().unwrap().len(), 2);
    }

    #[tokio::test]
    async fn deletion_before_ack_does_not_reuse_an_in_flight_id() {
        let (_dir, store, _sink, _flusher) = fixture();
        record(&store, "signin_completed");
        let in_flight = store.batch_for_sync(1).unwrap()[0].rowid;
        store.delete_optional().unwrap();
        record(&store, "signin_completed");
        store.advance_watermark(in_flight).unwrap();
        let remaining = store.batch_for_sync(1).unwrap();
        assert_eq!(remaining.len(), 1);
        assert!(remaining[0].rowid > in_flight);
    }

    #[tokio::test]
    async fn invalid_legacy_rows_do_not_block_or_leak_into_a_batch() {
        let (_dir, store, _sink, _flusher) = fixture();
        store
            .insert(
                "unknown_old_event",
                contract::Sensitivity::Optional,
                &json!({}),
            )
            .unwrap();
        store
            ...
            .unwrap();
        record(&store, "signin_completed");
        let batch = store.batch_for_sync(1).unwrap();
        assert_eq!(batch.len(), 1);
        assert_eq!(batch[0].properties, json!({"install_id": "ins_t"}));
        assert!(store.batch_for_sync(0).unwrap().is_empty());
    }
