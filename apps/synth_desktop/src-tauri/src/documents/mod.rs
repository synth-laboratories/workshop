...
mod tests {
    ...
    #[test]
    fn traversal_out_of_the_workspace_is_a_named_refusal() {
        let temp = tempfile::tempdir().unwrap();
        let workspace = temp.path().join("workspace");
        std::fs::create_dir_all(workspace.join("docs")).unwrap();
        std::fs::write(workspace.join("docs/readme.md"), "# hi").unwrap();
        std::fs::write(temp.path().join("secret.txt"), "no").unwrap();
        let workspace = workspace.canonicalize().unwrap();
        let allowed = roots(&[&workspace]);

        let error = resolve_in_scope(&allowed, "../secret.txt").unwrap_err();
        let failure = error
            .chain()
            .find_map(|cause| cause.downcast_ref::<StructuredFailure>())
            .expect("structured refusal");
        assert_eq!(failure.code, CODE_OUTSIDE_WORKSPACE);

        let (resolved, root) = resolve_in_scope(&allowed, "docs/readme.md").unwrap();
    ...
    fn a_symlink_escaping_the_workspace_is_refused() {
        ...
        let allowed = roots(&[&workspace]);
        #[cfg(unix)]
        assert!(resolve_in_scope(&allowed, "link.txt").is_err());
    }

    #[test]
    fn a_missing_file_inside_scope_resolves_and_is_named_missing() {
        let temp = tempfile::tempdir().unwrap();
        let workspace = temp.path().canonicalize().unwrap();
        let allowed = roots(&[&workspace]);
        let (resolved, root) = resolve_in_scope(&allowed, "not-here.md").unwrap();
        let record = record_for(&resolved, &root);
        assert!(!record.exists);
        assert_eq!(
            record.presentability(),
            Presentability::Unavailable(UnavailableReason::Missing)
        );
        assert_eq!(record.presentability().label(), "Missing");
    }

    #[test]
    fn a_binary_file_is_named_rather_than_rendered_as_mojibake() {
        let temp = tempfile::tempdir().unwrap();
        let workspace = temp.path().canonicalize().unwrap();
        std::fs::write(workspace.join("blob.bin"), [0_u8, 1, 2, 3, 255]).unwrap();
        let record = record_for(&workspace.join("blob.bin"), &workspace);
        assert!(!record.is_text);
        assert_eq!(
            record.presentability(),
            Presentability::Unavailable(UnavailableReason::NotText)
        );
    }

    #[test]
    fn breadcrumbs_carry_the_path_each_segment_lists() {
        let temp = tempfile::tempdir().unwrap();
        let workspace = temp.path().canonicalize().unwrap();
        std::fs::create_dir_all(workspace.join("docs/api")).unwrap();
        std::fs::write(workspace.join("docs/api/index.md"), "# api").unwrap();
        let record = record_for(&workspace.join("docs/api/index.md"), &workspace);
        let trail = record.breadcrumbs();
        assert_eq!(trail.len(), 4);
        assert_eq!(trail[1].label, "docs");
        assert!(trail[1].is_directory);
        assert_eq!(trail[3].label, "index.md");
        assert!(!trail[3].is_directory);
        assert_eq!(trail[2].path, workspace.join("docs/api").to_string_lossy());
    }

    #[test]
    fn language_ids_have_one_spelling() {
        assert_eq!(language_for(Path::new("/a/b.rs"), false), "rust");
        assert_eq!(language_for(Path::new("/a/README.md"), false), "markdown");
        assert_eq!(language_for(Path::new("/a/Dockerfile"), false), "docker");
        assert_eq!(language_for(Path::new("/a/mystery"), false), "text");
        assert_eq!(kind_for("markdown", false), DocumentKind::Markdown);
        assert_eq!(kind_for("rust", false), DocumentKind::Code);
        assert_eq!(kind_for("rust", true), DocumentKind::Directory);
    }

    #[test]
    fn a_listing_keeps_the_rows_it_cannot_open_and_says_why() {
        let temp = tempfile::tempdir().unwrap();
        let workspace = temp.path().canonicalize().unwrap();
        std::fs::write(workspace.join("readme.md"), "# hi").unwrap();
        std::fs::write(workspace.join("blob.bin"), [0_u8, 0, 0]).unwrap();
        std::fs::create_dir(workspace.join("src")).unwrap();

        let root = workspace.clone();
        let mut rows: Vec<DirectoryEntry> = std::fs::read_dir(&workspace)
            .unwrap()
            .flatten()
            .map(|entry| {
                let child = record_for(&entry.path(), &root);
        ...
        assert!(!blob.openable);
        assert_eq!(blob.reason.as_deref(), Some("Not text"));
        assert!(rows.iter().find(|row| row.name == "src").unwrap().openable);
    }
}
