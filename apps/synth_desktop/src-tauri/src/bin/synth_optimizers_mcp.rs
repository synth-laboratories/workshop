...
fn tools() -> Value {
    json!({"tools":[
        ...
        {"name":"optimizer_list_recipes","description":"List desktop-source-declared recipes plus product training recipes. Task GEPA/eval ids come from workshop.recipe.toml under configured source roots, never from a chat workspace.","inputSchema":{"type":"object","properties":{"session_ref":{"type":"string","description":"Optional run/visual ownership only; it does not affect recipe discovery."}},"additionalProperties":false}},
        {"name":"optimizer_start_recipe","description":"Start a desktop-source-declared or product recipe. GEPA/eval ids come from workshop.recipe.toml under configured source roots, never from a chat workspace. Local candidate-comparison eval.* still takes candidate_set_id. Baseline evals take container_id from container ensure.","inputSchema":{"type":"object","properties":{"recipe_id":{"type":"string"},"session_ref":{"type":"string","description":"Optional ownership for the run and visual; never a source selector."},"open_visual":{"type":"boolean"},"base_model":{"type":"string"},"dataset_shard":{"type":"string","enum":["train_a","train_b"]},"candidate_set_id":{"type":"string","description":"Required by pinned local candidate-comparison eval.* recipes. An id returned by optimizer_stage_eval_candidates, never a path."},"training_artifact_id":{"type":"string","description":"Verified local training artifact used as the CISPO warm start."},"container_id":{"type":"string","description":"Registered-container identity from container ensure. Required when multiple healthy pools advertise the same family."}},"required":["recipe_id"],"additionalProperties":false}},
        {"name":"optimizer_start_workflow","description":"Start one bounded workflow in one call. Task recipes are declared in workshop.recipe.toml under configured desktop source roots. Freshens registered-container capabilities, performs host approval and sidecar admission, creates the run, and opens its chat-owned visual.","inputSchema":{"type":"object","properties":{"recipe_id":{"type":"string"},"session_ref":{"type":"string","description":"Optional ownership for the run and visual; never a source selector."},"open_visual":{"type":"boolean"},"base_model":{"type":"string"},"dataset_shard":{"type":"string","enum":["train_a","train_b"]},"candidate_set_id":{"type":"string"},"training_artifact_id":{"type":"string","description":"Verified local training artifact used as the CISPO warm start."},"container_id":{"type":"string"}},"required":["recipe_id"],"additionalProperties":false}},
