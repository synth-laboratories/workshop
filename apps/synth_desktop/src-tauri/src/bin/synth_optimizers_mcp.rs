...
fn tools() -> Value {
    let mut manifest = json!({"tools":[
        ...
        {"name":"optimizer_evaluation_start","description":"The only approval path for executing an inline evaluation. Rebuild and revalidate the inline specification, emit Workshop's native clickable digest-bound paid-compute modal, wait for Approve with cap or Deny, reverify drift, start only the approved run, and attach its Workshop visual. idempotencyKey identifies this logical start: generate it once and reuse it verbatim after a timeout; use a new key for an intentional rerun. Do not replace this tool with prose asking the user to reply with approval.","inputSchema":{"type":"object","properties":{"containerId":{"type":"string"},"family":{"type":"string"},"policyNamespace":{"type":"string"},"policyName":{"type":"string"},"policyOverrides":{"type":"object"},"provider":{"type":"string"},"modelId":{"type":"string"},"seeds":{"type":"array","items":{"type":"integer"},"minItems":1},"maximumRollouts":{"type":"integer","minimum":1},"maximumModelCallsPerRollout":{"type":"integer","minimum":1},"maximumStepsPerRollout":{"type":"integer","minimum":1},"hardTotalCostUsd":{"type":"number","exclusiveMinimum":0},"sessionRef":{"type":"string"},"openVisual":{"type":"boolean"},"idempotencyKey":{"type":"string","minLength":1,"maxLength":128}},"required":["policyNamespace","policyName","provider","modelId","seeds","maximumRollouts","maximumModelCallsPerRollout","maximumStepsPerRollout","hardTotalCostUsd","idempotencyKey"],"additionalProperties":false}},
        ...
        {"name":"optimizer_reconcile_rhodes","description":"Attach Workshop visuals to an accepted Rhodes evaluation and resume durable event replay. Does not submit compute. Cleanup remains independently visible.","inputSchema":{"type":"object","properties":{"pool_id":{"type":"string"},"rollout_id":{"type":"string"},"open_visual":{"type":"boolean"}},"required":["pool_id","rollout_id"],"additionalProperties":false}},
...
fn call_tool(name: &str, args: &Value) -> Result<Value, String> {
    if name == "optimizer_manage" {
        ...
        let tool = match operation {
            ...
            "reconcile_rhodes" => "optimizer_reconcile_rhodes",
    ...
    match name {
        ...
        "optimizer_reconcile_rhodes" => request(
            "POST",
            "/v1/optimizers/reconcile_rhodes",
            Some(
                json!({"poolId":args.get("pool_id"),"rolloutId":args.get("rollout_id"),"openVisual":args.get("open_visual").and_then(Value::as_bool).unwrap_or(true)}),
            ),
        ),
...
mod tests {
    ...
    #[test]
    fn evaluation_start_manifest_requires_retry_identity() {
        let manifest = tools();
        let start = manifest["tools"]
            .as_array()
            .unwrap()
            .iter()
            .find(|tool| tool["name"] == "optimizer_evaluation_start")
            .unwrap();
