...
fn tools() -> Value {
    let mut catalog = json!({"tools":[
        ...
        {"name":"container_discover","description":"Discover container declarations from operator-approved project sources. Returns typed no_project_sources and project_sources_invalid readiness failures with the exact recovery path; it never scans arbitrary folders or ports.","inputSchema":{"type":"object","properties":{"query":{"type":"string","description":"Optional container family, declaration, or source query."}},"additionalProperties":false},"annotations":{"readOnlyHint":true,"destructiveHint":false,"idempotentHint":true,"openWorldHint":false}},
        {"name":"project_source_request","description":"Request a project source through Workshop's native picker. This grants nothing: after operator approval, call container_discover again to obtain the source_id and declarations.","inputSchema":{"type":"object","properties":{"path":{"type":"string","description":"Absolute folder proposed to the native picker."},"reason":{"type":"string","description":"Short operator-facing reason for requesting this source."},"capabilities":{"type":"array","items":{"type":"string"}},"attachToConversation":{"type":"boolean"}},"required":["path","reason"],"additionalProperties":false},"annotations":{"readOnlyHint":false,"destructiveHint":false,"idempotentHint":false,"openWorldHint":true}},
