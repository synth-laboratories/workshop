...
mod tests {
    ...
    fn version() -> RunVersion {
        RunVersion {
            helper_version: "1.0.0".into(),
            helper_identity: "Developer ID Application: Synth Laboratories".into(),
    ...
    #[test]
    fn a_secure_text_field_never_reaches_the_content_store() {
        let dir = tempdir().unwrap();
        let store = ContentStore::new(dir.path());
        let recorder = TrajectoryRecorder::new(store, version(), "run-1", "session-1");
        let captured = recorder
            .capture(&Observation {
                ax_text: Some(
                    "[3] AXSecureTextField value: hunter2\n[4] AXButton title: Sign In".into(),
                ),
                screenshot: None,
                element_count: Some(2),
            })
            .unwrap();
        let stored = ContentStore::new(dir.path())
            .get_bytes(KIND_AX_TREE, &captured.ax_digest.unwrap())
            .unwrap();
        let text = String::from_utf8(stored).unwrap();
        assert!(!text.contains("hunter2"), "{text}");
        assert!(text.contains(REDACTED));
        // Redaction must not eat the rest of the tree.
        assert!(text.contains("AXButton title: Sign In"), "{text}");
    }

    #[test]
    fn tokens_and_keys_are_redacted_wherever_they_appear() {
        for line in [
            "Authorization: Bearer abc.def.ghi",
            "[7] AXTextField value: sk-proj-0123456789abcdef",
            ...
            assert!(!redacted.contains("hunter2") || !line.contains("hunter2"));
        }
    }

    /// Over-redaction costs replay fidelity, so ordinary window text has to
    /// survive intact or the trajectory is useless for training.
    #[test]
    fn ordinary_window_text_survives() {
        for line in [
            "[1] AXButton title: Send",
            "[2] AXStaticText value: Meeting at 3pm with the platform team",
            "[3] AXTextField value: board@example.com",
            "[4] AXWindow title: Inbox — 12 unread",
        ] {
    ...
    #[test]
    fn a_step_records_refusals_and_coordinate_use() {
        let dir = tempdir().unwrap();
        ...
        let pixels = action(json!({"verb":"click","app":"com.figma.desktop","x":10.0,"y":20.0}));
        let step = recorder.record(
            ...
            None,
            RESULT_REFUSED,
            Some("app is not allowlisted".into()),
            "2026-08-16T00:00:00Z".into(),
            "2026-08-16T00:00:01Z".into(),
        );
        assert_eq!(step.result, RESULT_REFUSED);
        assert!(step.used_coordinates);
        assert_eq!(step.sequence, 1);
        assert_eq!(step.schema_version, TRAJECTORY_STEP_SCHEMA);
        assert_eq!(step.version.helper_version, "1.0.0");

        let indexed = action(json!({"verb":"click","app":"com.figma.desktop","element_index":2}));
        let next = recorder.record(
            &indexed,
            StateRef::default(),
            StateRef::default(),
            ...
            RESULT_OK,
            None,
            "2026-08-16T00:00:02Z".into(),
            "2026-08-16T00:00:03Z".into(),
        );
        assert_eq!(next.sequence, 2);
        assert!(!next.used_coordinates);
    }

    #[test]
    fn step_parameters_are_redacted_before_they_are_stored() {
        let dir = tempdir().unwrap();
        let mut recorder = TrajectoryRecorder::new(
            ContentStore::new(dir.path()),
            version(),
    ...
    #[test]
    fn identical_observations_are_stored_once() {
        let dir = tempdir().unwrap();
        ...
        let observation = Observation {
            ax_text: Some("[1] AXButton title: Send".into()),
            screenshot: None,
            element_count: Some(1),
        };
        let first = recorder.capture(&observation).unwrap();
        let second = recorder.capture(&observation).unwrap();
        assert_eq!(first.ax_digest, second.ax_digest);
    }

    #[test]
    fn a_key_named_field_is_dropped_whatever_its_value_looks_like() {
        let redacted = redact_value(&json!({"apiKey": "short", "text": "hello"}));
