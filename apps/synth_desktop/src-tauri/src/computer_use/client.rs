...
"#;
    ...
    async fn the_handshake_records_the_helper_version() {
        ...
        let client = HelperClient::spawn(&fake_helper(dir.path(), ECHO_SERVER), "nonce", None)
            .await
            .unwrap();
    ...
    async fn unsolicited_messages_are_skipped_while_waiting_for_an_answer() {
        let dir = tempdir().unwrap();
        let mut client = HelperClient::spawn(&fake_helper(dir.path(), ECHO_SERVER), "nonce", None)
            .await
            .unwrap();
...
"#;
        let mut client = HelperClient::spawn(&fake_helper(dir.path(), server), "nonce", None)
            .await
            .unwrap();
        let error = client
...
"#;
    ...
    async fn spawning_a_helper_that_is_not_there_fails_with_its_path() {
        ...
        let error = match HelperClient::spawn(&missing, "nonce", None).await {
