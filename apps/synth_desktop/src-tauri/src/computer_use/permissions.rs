...
mod tests {
    ...
    fn observed(pairs: &[(&str, GrantState)]) -> Vec<(String, GrantState)> {
        pairs
            .iter()
    ...
    #[test]
    fn every_grant_renders_a_row_even_when_the_helper_has_reported_nothing() {
        let rows = rows(&[]);
        assert_eq!(rows.len(), CATALOG.len());
        assert_eq!(rows[0].id, ACCESSIBILITY);
        assert_eq!(rows[0].label, "Accessibility");
        assert_eq!(rows[0].state, STATE_NOT_DETERMINED);
        // Apple Events is asked per app, so it has no global answer to show.
        assert_eq!(rows[2].state, STATE_NOT_APPLICABLE);
        assert!(rows.iter().all(|row| row.settings_url.is_some()));
    }

    #[test]
    fn a_refusal_names_the_grants_that_are_actually_missing() {
        assert_eq!(
            missing(&observed(&[
                (ACCESSIBILITY, GrantState::Granted),
                (SCREEN_RECORDING, GrantState::Denied),
            ])),
            vec![SCREEN_RECORDING.to_string()]
        );
        assert!(missing(&observed(&[
            (ACCESSIBILITY, GrantState::Granted),
            (SCREEN_RECORDING, GrantState::Granted),
        ]))
        .is_empty());
    }

    /// Automation is granted per app at first use, so waiting for it up front
    /// would leave the plugin permanently short of ready.
    #[test]
    fn apple_events_never_blocks_readiness() {
        let all_granted = observed(&[
            (ACCESSIBILITY, GrantState::Granted),
            (SCREEN_RECORDING, GrantState::Granted),
            (APPLE_EVENTS, GrantState::NotDetermined),
        ]);
        assert!(is_ready(&all_granted));
        assert_eq!(refine_phase("ready", &all_granted), "ready");
    }

    #[test]
    fn an_installed_helper_without_grants_reports_needs_permissions() {
        let none = observed(&[]);
        assert_eq!(refine_phase("installed", &none), PHASE_NEEDS_PERMISSIONS);
        assert_eq!(refine_phase("ready", &none), PHASE_NEEDS_PERMISSIONS);
        assert_eq!(refine_phase("starting", &none), PHASE_NEEDS_PERMISSIONS);
    }

    /// "Downloading" and "error" are more specific than "needs permission";
    ...
    #[test]
    fn grant_state_survives_a_wire_round_trip_and_degrades_safely() {
        for state in [
            GrantState::Granted,
            GrantState::Denied,
            GrantState::NotDetermined,
            ...
            assert_eq!(GrantState::from_wire(state.wire()), state);
        }
        // An unknown value must not read as granted.
        assert_eq!(
            GrantState::from_wire("something-new"),
            GrantState::NotDetermined
        );
    }
}
