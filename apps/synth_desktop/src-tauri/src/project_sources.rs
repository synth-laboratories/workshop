...
pub async fn refresh(db: &Arc<Database>) -> Result<()> {
    ...
    // Recipe discovery persists its own provenance rows; a declaration error in
    // one source must not make the container refresh look like it failed.
    if let Err(error) = crate::optimizers::workspace_recipe::refresh_sources(db).await {
        eprintln!("synth-desktop: recipe catalog refresh reported {error}");
    }
    Ok(())
}
