...
impl ModelPerformanceRepository {
    ...
    pub async fn record(&self, sample: ModelPerformanceSample) -> Result<()> {
        self.db.run(move |conn| record(conn, sample)).await
    }
...
mod tests {
    ...
    fn sample(request: &str, tps: f64) -> ModelPerformanceSample {
    ...
    async fn records_idempotently_and_returns_model_percentiles() {
        ...
        let r = ModelPerformanceRepository::new(s.database().clone());
        for (id, tps) in [("one", 10.), ("two", 20.), ("three", 30.), ("three", 30.)] {
            r.record(sample(id, tps)).await.unwrap()
        }
        let v = r.summaries().await.unwrap();
    ...
    async fn omits_failed_and_tokenless_rows() {
        ...
        let s = Storage::open(t.path()).unwrap();
        let r = ModelPerformanceRepository::new(s.database().clone());
        let mut x = sample("failed", 40.);
        x.status = "failed".into();
        r.record(x).await.unwrap();
        let mut x = sample("tokenless", 40.);
        x.output_tokens = None;
        x.observed_output_tps = None;
        r.record(x).await.unwrap();
        assert!(r.summaries().await.unwrap().is_empty());
    }
}
