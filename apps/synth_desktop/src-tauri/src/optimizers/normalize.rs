...
mod tests {
    ...
    #[test]
    fn normalizes_gepa_oss_event_feed_record() {
        let raw = json!({
            ...
            "message": "accepted",
            "timestamp": "2026-08-09T15:00:25Z",
            "fields": {
                "run_id": "gepa_local_1",
        ...
        assert_eq!(event.optimizer_run_id, "gepa_local_1");
        assert_eq!(event.sequence_number, 3);
        assert_eq!(event.item.as_ref().unwrap()["id"], "cand_m1");
    }

    #[test]
    fn normalizes_native_gepa_runtime_job_with_usage() {
        let raw = json!({
            "_seq": 12,
            "type": "runtime.job.completed",
            "ts": "2026-08-10T01:21:40Z",
            "message": "Runtime job completed",
            "fields": {
                "run_id": "gepa_native_1",
                "rollout_count": 4,
                "cost_usd": 0.02,
                "wall_seconds": 2.5,
        ...
        assert_eq!(event.occurred_at, "2026-08-10T01:21:40Z");
    ...
    fn projects_native_gepa_frontier_into_visual_cells() {
        ...
        assert_eq!(snapshot["bestScore"], json!(0.5));
        assert_eq!(snapshot["cells"][0]["candidateId"], json!("cand_seed"));
        assert_eq!(snapshot["cells"][0]["accent"], json!(true));
    }

    #[test]
    fn normalizes_hosted_goex_event() {
        let raw = json!({
            "run_id": "goex_cloud_1",
            "event_type": "theme.updated",
            "_seq": 12,
            "created_at": "2026-08-09T15:10:20Z",
            "payload": {"theme": "wood", "saturation": 0.4},
            "algorithm": "go-ex"
        });
        let event = normalize_event(&raw, "fallback", "go-ex").unwrap();
        assert_eq!(event.algorithm_id, "go-ex");
        assert_eq!(event.event_type, "go-ex.theme.updated");
        assert_eq!(event.sequence_number, 12);
        assert_eq!(event.delta.get("theme").unwrap(), "wood");
    }
