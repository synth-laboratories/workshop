...
mod tests {
    ...
    #[test]
    fn no_run_exists_before_approval() {
        let mut draft = RunDraft::new("draft-1", AlgorithmKind::Eval, "sha256:abc", "{}", "now");
        let error = AdmissionCommit::from_approved_draft(
            &draft,
            "run-1",
        ...
        .unwrap_err();
        assert_eq!(error.code, KernelErrorCode::DraftNotApproved);
        ...
        .unwrap();
        assert_eq!(commit.run_id, "run-1");
        assert_eq!(commit.spec_digest, "sha256:abc");
    }
}
