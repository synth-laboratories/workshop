...
mod tests {
    ...
    #[test]
    fn work_item_identity_is_required_before_dispatch() {
        let error = WorkItem::planned(" ", WorkItemKind::EvalTrial).unwrap_err();
        assert_eq!(error.code, KernelErrorCode::WorkItemIdentityMissing);
        let mut item = WorkItem::planned("trial:0:0:0", WorkItemKind::EvalTrial).unwrap();
        item.transition(WorkItemLifecycle::Queued).unwrap();
        item.bind_external_ref("harbor-rollout-9".into()).unwrap();
        assert_eq!(item.external_ref.as_deref(), Some("harbor-rollout-9"));
    }

    #[test]
    fn empty_work_is_unavailable_not_zero() {
        let summary = WorkSummary::from_items(&[], "trials", true);
        assert_eq!(summary.planned, None);
        assert_eq!(summary.succeeded, None);
        assert_eq!(summary.unit.as_deref(), Some("trials"));
    }
}
