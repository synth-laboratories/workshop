...
pub(crate) fn critical_observations(events: &[Value]) -> serde_json::Map<String, Value> {
    ...
    for event in events {
        let field = match event.get("event_type").and_then(Value::as_str) {
            ...
            Some("rollout.sealed_trace_attached") => "lastSealedTraceAttachment",
            Some("rollout.imported_trace_published" | "rollout.imported_trace_observation") => "lastImportedTracePublication",
            Some("rollout.budget_settled") => "lastBudgetSettlement",
            Some("rollout.provider_trace_published") => "lastProviderTracePublication",
            Some("rollout.native_agent_trace_published") => "lastNativeAgentTracePublication",
