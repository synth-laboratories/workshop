...
mod tests {
    ...
    fn key_order_does_not_change_the_digest() {
        let first = CanonicalJson::new(json!({"b": 1, "a": {"d": 2, "c": 3}})).unwrap();
        let second = CanonicalJson::new(json!({"a": {"c": 3, "d": 2}, "b": 1})).unwrap();
        assert_eq!(first.to_canonical_string(), second.to_canonical_string());
        assert_eq!(first.digest(), second.digest());
        assert_eq!(first.to_canonical_string(), r#"{"a":{"c":3,"d":2},"b":1}"#);
    }

    #[test]
    fn array_order_is_data_and_is_preserved() {
        let ascending = CanonicalJson::new(json!({"seeds": [780000, 780001]})).unwrap();
        let descending = CanonicalJson::new(json!({"seeds": [780001, 780000]})).unwrap();
        assert_ne!(
            ascending.digest(),
            descending.digest(),
            "reordering seeds is a different specification, not the same one"
        );
    }

    #[test]
    fn digests_are_stable_across_runs_and_encodings() {
        let value = json!({
            "container": "nanohorizon-craftax",
            "seeds": [780000, 780001, 780002, 780003, 780004],
            "costCeilingMicros": 2_450_000u64,
        });
        let once = CanonicalJson::new(value.clone()).unwrap().digest();
        // Round-tripping through text must not change the digest.
        let text = serde_json::to_string(&value).unwrap();
        let reparsed: Value = serde_json::from_str(&text).unwrap();
        let twice = CanonicalJson::new(reparsed).unwrap().digest();
        assert_eq!(once, twice);
        assert!(once.as_str().starts_with("sha256:"));
    }

    #[test]
    fn negative_zero_folds_and_integral_floats_stay_floats() {
        let folded = CanonicalJson::new(json!({"value": -0.0})).unwrap();
        assert_eq!(folded.to_canonical_string(), r#"{"value":0}"#);
        let integral = CanonicalJson::new(json!({"value": 2.0})).unwrap();
        assert_eq!(integral.to_canonical_string(), r#"{"value":2.0}"#);
    }

    #[test]
    fn strings_escape_control_characters_rather_than_dropping_them() {
        let text = format!("a\nb{}c\"d", '\u{1}');
        let escaped = CanonicalJson::new(json!({ "note": text })).unwrap();
        assert_eq!(
            escaped.to_canonical_string(),
            r#"{"note":"a\nb\u0001c\"d"}"#
        );
    }
