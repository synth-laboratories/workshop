...
mod tests {
    use super::*;
    use crate::optimizers::admission::ids::non_zero_u32;
    use serde_json::json;
    use std::num::NonZeroU64;

    pub(crate) fn sample_recipe() -> InlineRecipe {
        InlineRecipe {
            ...
            evaluator: EvaluatorSpec::ContainerDeclared {
                evaluator_id: EvaluatorId::new("craftax.achievements").unwrap(),
                evaluator_version: "1.2.0".into(),
                scoring_digest: super::super::canonical::digest_bytes(b"scoring"),
            },
            policy: PolicyPin::new(
                "nanohorizon",
                "glm-5.3-flash",
                PolicyRevision::new("rev-9").unwrap(),
                CanonicalJson::new(json!({"max_calls": 10, "max_steps": 2000})).unwrap(),
            ),
            model: ModelPin {
                provider: ProviderId::new("openrouter").unwrap(),
                model_id: ModelId::new("z-ai/glm-5.3-flash").unwrap(),
            },
            rollout_plan: RolloutPlan {
                seeds: (780000..780005).map(Seed).collect(),
                maximum_rollouts: RolloutCount(non_zero_u32(5, "maximum_rollouts").unwrap()),
            },
            resource_limits: ResourceLimits {
                maximum_model_calls_per_rollout: ModelCallCount(
                    non_zero_u32(10, "maximum_model_calls_per_rollout").unwrap(),
                ),
                maximum_steps_per_rollout: StepCount(
                    non_zero_u32(2000, "maximum_steps_per_rollout").unwrap(),
                ),
                hard_total_cost_micros: CostMicros(NonZeroU64::new(2_450_000).unwrap()),
            },
            credential_route: CredentialRoute::WorkshopSecretsProxy {
                provider: ProviderId::new("openrouter").unwrap(),
                capability_scope: CredentialCapabilityScope::new(
                    ["chat.completions".to_string()],
                    900,
                ),
            },
            output_contract: OutputContract::new(true, true, true, ["reward.get".to_string()]),
        }
    }

    pub(crate) fn sample_spec() -> ExecutionSpec {
        ExecutionSpec {
    ...
    #[test]
    fn a_specification_round_trips_through_json_unchanged() {
        let spec = sample_spec();
        let encoded = serde_json::to_value(&spec).unwrap();
        let decoded: ExecutionSpec = serde_json::from_value(encoded).unwrap();
        assert_eq!(spec, decoded);
        assert_eq!(spec.digest().unwrap(), decoded.digest().unwrap());
    }

    #[test]
    fn the_digest_changes_when_any_pinned_input_changes() {
        let baseline = sample_spec();
        let baseline_digest = baseline.digest().unwrap();

        let mut different_model = sample_spec();
        different_model.recipe.model.model_id = ModelId::new("z-ai/glm-5.3-pro").unwrap();
        assert_ne!(baseline_digest, different_model.digest().unwrap());

        let mut different_seed = sample_spec();
        different_seed.recipe.rollout_plan.seeds[0] = Seed(999999);
        assert_ne!(baseline_digest, different_seed.digest().unwrap());

        let mut different_ceiling = sample_spec();
        different_ceiling
            .recipe
            .resource_limits
            .hard_total_cost_micros = CostMicros(NonZeroU64::new(2_450_001).unwrap());
        assert_ne!(baseline_digest, different_ceiling.digest().unwrap());

        let mut different_declaration = sample_spec();
        different_declaration.recipe.container.declaration_digest =
            DeclarationDigest::new("sha256:other").unwrap();
        assert_ne!(baseline_digest, different_declaration.digest().unwrap());
    }

    #[test]
    fn the_same_specification_from_either_source_differs_only_in_its_label() {
        let inline = sample_spec();
        let catalog = ExecutionSpec {
            source_kind: RecipeSourceKind::Catalog,
            catalog_recipe_id: Some(RecipeId::new("nanohorizon.craftax.v1").unwrap()),
            recipe: sample_recipe(),
        };
        // The executable content is identical...
        assert_eq!(inline.recipe, catalog.recipe);
        // ...but the provenance is recorded, so the UI can label a catalog run
        // as one without inferring it from the presence of an id.
        assert_ne!(inline.digest().unwrap(), catalog.digest().unwrap());
        assert_eq!(catalog.source_kind, RecipeSourceKind::Catalog);
    }

    #[test]
    fn a_policy_pin_detects_configuration_rewritten_underneath_it() {
        let mut pin = sample_recipe().policy;
        assert!(pin.digest_matches());
        pin.configuration = CanonicalJson::new(json!({"max_calls": 11})).unwrap();
        assert!(
            !pin.digest_matches(),
            "a rewritten configuration must not quietly become the new truth"
        );
    }

    #[test]
    fn an_unknown_protocol_is_not_coerced_to_the_known_one() {
        assert_eq!(
    ...
    #[test]
    fn capability_scope_and_output_contract_sort_their_operations() {
        // Sorting matters because these participate in the digest: the same
        // scope written in a different order must not read as a different one.
        let scope = CredentialCapabilityScope::new(
