...
mod tests {
    ...
    fn upsert_and_search_round_trip() {
        ...
        assert!(imported.inference_responses);
        assert!(imported.checkpoint_id.starts_with("sha256:"));
        let patched = patch_local_lora(
            &conn,
            &imported.checkpoint_id,
            &SavedLoraPatchRequest {
                name: Some("Banking77 night".into()),
                description: Some("notes".into()),
                tags: Some(vec!["eval".into()]),
            },
        )
        .unwrap();
        assert_eq!(patched.name, "Banking77 night");
        assert_eq!(patched.tags, vec!["eval"]);
        let page = search_local_loras(&conn, &SavedLoraCheckpointQuery::default()).unwrap();
        assert_eq!(page.len(), 1);
        archive_local_lora(&conn, &imported.checkpoint_id).unwrap();
        let hidden = search_local_loras(&conn, &SavedLoraCheckpointQuery::default()).unwrap();
    ...
    fn hosted_overlay_survives_cloud_patch_failure() {
        ...
        let patched = overlay_hosted_lora(
            ...
            &base.checkpoint_id,
            &SavedLoraPatchRequest {
                name: Some("Renamed locally".into()),
                description: Some("overlay notes".into()),
                tags: Some(vec!["eval".into()]),
            },
            base.clone(),
        )
        .unwrap();
        assert_eq!(patched.checkpoint_id, base.checkpoint_id);
        assert_eq!(patched.name, "Renamed locally");
        assert_eq!(patched.description, "overlay notes");
        assert_eq!(patched.tags, vec!["eval"]);
        assert_eq!(patched.placement, "hosted");
        assert_eq!(patched.metadata["localOverlay"], json!(true));

        let mut items = vec![hosted_checkpoint(
            "Cloud name",
            "cloud notes",
            vec!["prod".into()],
        )];
        let overlays = list_hosted_overlays(&conn).unwrap();
        apply_hosted_overlay(&mut items[0], overlays.get("ckpt_hosted_1").unwrap());
        assert_eq!(items[0].name, "Renamed locally");
        ...
        overlay_hosted_lora(
            ...
            &SavedLoraPatchRequest {
                name: None,
                description: Some("second note".into()),
                tags: None,
            },
            items[0].clone(),
        )
        .unwrap();
        let stacked = get_hosted_overlay(&conn, "ckpt_hosted_1").unwrap().unwrap();
        assert_eq!(stacked.name.as_deref(), Some("Renamed locally"));
        assert_eq!(stacked.description.as_deref(), Some("second note"));
    ...
    fn hosted_checkpoint(name: &str, description: &str, tags: Vec<String>) -> SavedLoraCheckpoint {
        SavedLoraCheckpoint {
            schema_version: "saved_lora_checkpoint.v1".into(),
            checkpoint_id: "ckpt_hosted_1".into(),
            org_id: "org".into(),
            owner_user_id: None,
            visibility: "private".into(),
            name: name.into(),
            description: description.into(),
            provider: "tinker".into(),
            checkpoint_kind: "inference".into(),
            provider_checkpoint_reference: Some("tinker://sampler".into()),
            ...
            placement: "hosted".into(),
            inference_chat_completions: true,
            inference_responses: true,
            tags,
            metadata: json!({}),
            created_at: Some("2026-08-20T12:00:00Z".into()),
            updated_at: Some("2026-08-20T12:00:00Z".into()),
            archived_at: None,
        }
    }
}
