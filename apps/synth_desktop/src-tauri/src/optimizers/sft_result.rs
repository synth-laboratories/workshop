...
pub async fn materialize_sft_result(
    ...
    let mut events = service
        .events_after(run.id.clone(), 0, Some(2_000))
        .await?;
    if events.is_empty() {
...
fn config_from(
    ...
    let rank = summary
        ...
        .or_else(|| manifest.and_then(|value| value.get("rank")).and_then(Value::as_u64))
...
fn training_tokens(events: &[OptimizerEventEnvelope]) -> Option<u64> {
    events.iter().rev().find_map(|event| {
        json_u64(event.delta.get("tokens"))
            .or_else(|| event.usage_delta.as_ref().and_then(|delta| json_u64(delta.get("training_tokens"))))
    })
}

fn last_u64(events: &[OptimizerEventEnvelope], keys: &[&str]) -> Option<u64> {
    events.iter().rev().find_map(|event| {
        keys.iter().find_map(|key| json_u64(event.delta.get(*key)))
    })
}
...
pub fn sft_milestone_kind(event_type: &str, level: Option<&str>) -> Option<&'static str> {
    ...
    match event_type {
        ...
        "sft.checkpoint.created" | "sft.checkpoint.ready" | "sft.checkpoint.selected"
...
mod tests {
    ...
    fn evt(event_type: &str, seq: u64, delta: Value, usage: Option<Map<String, Value>>) -> OptimizerEventEnvelope {
    ...
    fn sft_result_does_not_require_best_candidate_and_nulls_missing_metrics() {
        ...
        assert_eq!(result["metrics"]["validationLoss"]["coverage"], "unsupported");
        ...
        assert_eq!(result["usage"]["inferenceTokens"]["coverage"], "unsupported");
