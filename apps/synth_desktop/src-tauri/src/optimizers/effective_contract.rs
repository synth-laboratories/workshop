...
mod tests {
    ...
    fn template(id: &str, genre: &str, tags: &[&str]) -> TemplateMeta {
        TemplateMeta {
            ...
            version: None,
            description: None,
            family: tags.first().map(|value| (*value).into()),
    ...
    fn templates() -> Vec<TemplateMeta> {
        vec![
            template("live.example.v1", "live", &["example"]),
            template(PRIMARY_FALLBACK, "report", &[]),
            template(TRACE_FALLBACK, "trace", &[]),
        ]
    }

    #[test]
    fn declared_family_and_media_are_intersected_without_family_code() {
        let contract = negotiate(
            ...
            &json!({
                "liveEval": {
                    "family": "example",
                    "artifactMediaTypes": ["video/mp4", "text/html"]
                }
            }),
            &templates(),
        )
        ...
        assert_eq!(
            ...
            Some("live.example.v1")
        );
        assert_eq!(
            contract.primary_visual.state,
            EffectiveVisualState::FamilyMatched
        );
        assert_eq!(
            contract.trace_visual.template_id.as_deref(),
            Some(TRACE_FALLBACK)
        );
        assert_eq!(contract.artifact_media_types, vec!["video/mp4"]);
    }

    #[test]
    fn contradictory_or_unregistered_declarations_are_typed_refusals() {
        let error = negotiate(
            "run-1",
            "container-1",
            Some("one"),
            &json!({"liveEval": {"family": "two"}}),
            &templates(),
        )
        .unwrap_err();
        ...
        let error = negotiate(
            ...
            "container-1",
            Some("example"),
            &json!({"liveEval": {"templateId": "missing.v1"}}),
            &templates(),
        )
        .unwrap_err();
        let refusal = error.downcast_ref::<ContractRefusal>().unwrap();
