...
pub(crate) fn tools() -> Value {
    let mut result = json!({
        "tools": [
            {"name":"visual_manage","description":"Use author-synth-diagrams; do not call MCP resources. Short display_name. Chart: arguments.spec; Mermaid: arguments.content. Then show/review/revise/mark_ready.","inputSchema":{"type":"object","properties":{"operation":{"type":"string","description":"Visual operation."},"arguments":{"type":"object","description":"Operation arguments. Creation operations should include display_name.","additionalProperties":true}},"required":["operation","arguments"],"additionalProperties":false}},
    ...
    if let Some(items) = result.get_mut("tools").and_then(Value::as_array_mut) {
        items.extend(crate::contract::capabilities::migrated_tools());
        ...
        for tool in items {
            ...
            if let Some(object) = tool.as_object_mut() {
                // Migrated operations carry domain-owned annotations. Keep
                // the legacy inference only for unmigrated declarations.
                if object.contains_key("annotations") {
                    continue;
                }
                object.insert(
                    "annotations".into(),
    ...
    if let Some(items) = result["tools"].as_array_mut() {
        for tool in items {
            if tool["name"] == "visual_manage" { continue; }
            tool["inputSchema"]["properties"]["session_id"] = json!({"type":"string","description":"Existing Workshop task correlation for task-owned experiment/document/template operations. Shared visuals do not require it."});
        }
    }
    result
}
    ...
    session_env
        ...
        .ok_or_else(|| {
            json!({
                ...
                "retryable": false,
                "remediation": "This operation needs an existing Workshop task identity. Pass session_id through the public server, or use a hosted server with SYNTH_SESSION_ID bound. Shared visual creation, charts and forks do not require a task."
            })
            .to_string()
        })
}
...
pub(crate) fn call_tool(name: &str, args: &Value) -> Result<Value, String> {
    ...
    call_scoped(name, args, env::var("SYNTH_SESSION_ID").ok(), false)
}

pub(crate) fn call_public(name: &str, args: &Value) -> Result<Value, String> {
    let session = args.get("session_id").and_then(Value::as_str).map(str::to_owned);
    call_scoped(name, args, session, true)
}

fn call_scoped(name: &str, args: &Value, session_env: Option<String>, public: bool) -> Result<Value, String> {
    let workspace_owned = public && session_env.is_none();
    match name {
        "visual_manage" => {
            let operation = args
                .get("operation")
            ...
            call_scoped(managed_tool_name(operation)?, arguments, session_env, public)
        }
        "visual_list_templates" => {
            let query: crate::contract::capabilities::ListVisualTemplatesRequest =
        ...
        "visual_create" | "visual_create_from_template" => {
            ...
            let session_id = if workspace_owned { None } else { Some(require_session_identity(&session_env, "create a visual")?) };
        ...
        "visual_chart" => {
            ...
            let visual = match existing {
                ...
                None => {
                    let session_id = if workspace_owned { None } else { Some(require_session_identity(&session_env, "create a chart")?) };
                    request(
                        "POST",
                        "/v1/visuals",
                        Some(json!({
        ...
        "visual_fork" => {
            ...
            // what this replaces.
            let session_id = if workspace_owned { None } else { Some(require_session_identity(&session_env, "fork a visual")?) };
            let forked = request(
                "POST",
