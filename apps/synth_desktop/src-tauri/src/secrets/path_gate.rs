...
mod tests {
    ...
    fn empty_roots_fail_closed() {
        let error = gate_workspace_file(&[], "wsroot_missing", ".env").unwrap_err();
        assert!(error
            .to_string()
            .contains(CREDENTIAL_LOCATOR_UNAPPROVED_WORKSPACE));
    }

    #[test]
    fn parent_components_are_refused() {
        let root = tempfile::tempdir().unwrap();
        let reference = workspace_root_ref(&root.path().canonicalize().unwrap());
        let error = gate_workspace_file(&[root.path().into()], &reference, "../.env").unwrap_err();
        assert!(error.to_string().contains(CREDENTIAL_PATH_ESCAPE));
    }
    ...
    fn symlink_out_is_refused() {
        ...
        symlink(outside.path(), root.path().join(".env")).unwrap();
        let reference = workspace_root_ref(&root.path().canonicalize().unwrap());
        let error = gate_workspace_file(&[root.path().into()], &reference, ".env").unwrap_err();
        assert!(error.to_string().contains(CREDENTIAL_PATH_ESCAPE));
    }
    ...
    #[test]
    fn variable_names_cannot_start_with_a_digit() {
        assert!(is_valid_env_variable("_OPENAI_API_KEY"));
        assert!(is_valid_env_variable("OPENAI_API_KEY2"));
        assert!(!is_valid_env_variable("2OPENAI_API_KEY"));
        assert!(!is_valid_env_variable("OPENAI-API-KEY"));
    }

    #[test]
    fn broad_discovery_patterns_are_refused() {
        let root = tempfile::tempdir().unwrap();
        let reference = workspace_root_ref(&root.path().canonicalize().unwrap());
        let error = gate_workspace_file(&[root.path().into()], &reference, ".env*").unwrap_err();
        assert!(error
            .to_string()
            .contains(CREDENTIAL_LOCATOR_BROAD_DISCOVERY));
    }
}
