...
async fn interrupt_terminates_non_cooperative_tool_tree_and_allows_a_new_turn() {
    ...
    assert!(
        !after_stop
            .iter()
            .any(|event| event.payload.to_string().contains("LATE_CANCEL_SENTINEL")),
...
fn completed_envelope_with_a_failed_turn_is_normalized_to_failed() {
    ...
    assert_eq!(
        normalized_turn_method("turn/completed", &json!({"turn": {"status": "cancelled"}})),
        "turn/interrupted"
    );
}
