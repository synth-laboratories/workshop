...
impl ApprovalKind {
    ...
    /// A hazard action commits content on the operator's behalf, while
    /// persisting a visual template leaves code behind that the app compiles at
    /// every launch. Consent is about that exact payload — a previous yes or a
    /// permissive shell policy is not a substitute for either grant.
    ///
