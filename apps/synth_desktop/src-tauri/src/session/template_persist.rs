...
impl PersistConsent {
    ...
    /// A consent no human settled, for tests that exercise the writers rather
    /// than the gate. Test-only on purpose: the whole point of the private
    /// fields is that production code has exactly one way in.
    #[cfg(test)]
    pub(crate) fn granted_for_test(request: &PersistRequest) -> Self {
        Self {
...
mod tests {
    ...
    const SHELL: &[u8] = b"export default function Shell() { return null; }\n";
    ...
    fn request() -> PersistRequest {
        PersistRequest::new(
            ...
            "save",
            SHELL,
            PersistDisposition::default(),
            Path::new("/root/visuals/templates/user.reward.v1"),
        )
    }

    #[test]
    fn a_request_measures_the_source_it_is_about_to_persist() {
        let request = request();
        assert_eq!(request.byte_size, SHELL.len() as u64);
        assert!(request.source_digest.starts_with("sha256:"));
        let kind = request.kind();
        assert_eq!(kind.name(), "visual_template_persist");
        assert!(kind.requires_human());
    }

    #[test]
    fn fork_and_overwrite_reach_the_card_as_different_questions() {
        let mut forked = request();
        forked.disposition.forked_from = Some("analysis.visual.v1".into());
        let payload = forked.kind().safe_payload("approval-1");
        assert_eq!(payload["forkedFrom"], "analysis.visual.v1");
        assert_eq!(payload["overwrites"], false);

        let mut replaced = request();
        replaced.disposition.overwrites = true;
        let payload = replaced.kind().safe_payload("approval-2");
        assert_eq!(payload["overwrites"], true);
        assert!(payload["forkedFrom"].is_null());

        // A fork that lands on an id somebody already wrote is both, and the
        // card has to say both -- that is why these are two fields.
        let mut both = request();
        both.disposition = PersistDisposition {
            overwrites: true,
            forked_from: Some("analysis.visual.v1".into()),
        };
        let payload = both.kind().safe_payload("approval-3");
        assert_eq!(payload["overwrites"], true);
        assert_eq!(payload["forkedFrom"], "analysis.visual.v1");
    }

    /// An approval names one template and one digest. Spending it on anything
    /// else is the hole the binding exists to close.
    #[test]
    fn a_consent_does_not_authorize_a_different_write() {
        let approved = request();
        let consent = PersistConsent::granted_for_test(&approved);
        consent.bind(&approved).unwrap();

        let mut other_id = request();
        other_id.template_id = "user.other.v1".into();
        assert!(consent.bind(&other_id).is_err());

        let other_bytes = PersistRequest::new(
            "user.reward.v1",
            "user",
            "save",
            b"export default function Different() { return null; }\n",
            PersistDisposition::default(),
            Path::new("/root/visuals/templates/user.reward.v1"),
        );
        assert!(consent.bind(&other_bytes).is_err());
    }

    /// No policy may settle this, `never` included — the permissive setting is
    /// honored everywhere else in Workshop precisely so that the places it is
    /// not honored can be listed and defended.
    #[test]
    fn no_approval_policy_auto_settles_persisting_template_code() {
        for policy in ["never", "on-request", "untrusted"] {
            assert!(
                super::super::approval_policy::auto_decision(policy, &request().kind())
                    .unwrap()
                    .is_none(),
                "`{policy}` auto-settled a template persist"
            );
        }
    }
}
