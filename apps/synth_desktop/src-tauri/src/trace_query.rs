...
pub fn result_digest(query_ast: &Value, result_ids: &[String]) -> String {
    ...
    hasher.update(serde_json::to_vec(query_ast).unwrap_or_default().as_slice());
...
mod tests {
    ...
    fn an_empty_query_is_bounded_and_deterministically_ordered() {
        ...
        assert!(compiled
            .sql
            .contains("ORDER BY started_at DESC, trace_digest ASC"));
    ...
    fn every_value_is_bound_and_never_interpolated() {
        let query = TraceQuery {
            ...
            r#where: Some(TraceWhere {
                ...
                reward: Some(RangeF64 {
                    gte: Some(0.5),
                    lte: None,
                }),
                started: Some(TimeRange {
                    after: Some("2026-08-14T00:00:00Z".into()),
                    before: None,
                }),
                ..TraceWhere::default()
            }),
            order_by: vec![TraceOrder {
                field: "reward".into(),
                direction: "asc".into(),
            }],
            limit: Some(25),
        ...
        assert!(compiled
            .sql
            .contains("ORDER BY reward ASC, trace_digest ASC"));
    ...
    fn only_allow_listed_fields_can_be_named() {
        let bad_order = TraceQuery {
            order_by: vec![TraceOrder {
                field: "trace_digest; DROP TABLE trace_index".into(),
                direction: "asc".into(),
            }],
            ..Default::default()
        };
        ...
        let bad_direction = TraceQuery {
            order_by: vec![TraceOrder {
                field: "reward".into(),
                direction: "asc; DELETE FROM traces".into(),
            }],
            ..Default::default()
        };
    ...
    fn identity_covers_the_query_and_the_rows_in_order() {
        ...
        assert_ne!(
            a,
            result_digest(&json!({"limit": 11}), &["d1".into(), "d2".into()])
        );
