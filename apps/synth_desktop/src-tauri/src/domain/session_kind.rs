...
mod tests {
    ...
    #[test]
    fn round_trips_serde_and_parse() {
        assert_eq!(SessionKind::parse("codex").unwrap(), SessionKind::Codex);
        assert_eq!(SessionKind::parse("intern").unwrap(), SessionKind::Intern);
        assert!(SessionKind::parse("local").is_err());
        assert_eq!(
            serde_json::to_string(&SessionKind::Intern).unwrap(),
            "\"intern\""
        );
    }

    #[test]
    fn recovers_kind_from_legacy_target_bags() {
        assert_eq!(
            SessionKind::from_target_json(&json!({"kind": "intern", "mode": "sync"})),
            SessionKind::Intern
        );
        assert_eq!(
            SessionKind::from_target_json(&json!({"kind": "codex"})),
            SessionKind::Codex
        );
        assert_eq!(
