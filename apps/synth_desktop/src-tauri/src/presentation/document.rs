...
mod tests {
    ...
    fn record(path: &str, exists: bool, is_text: bool, kind: DocumentKind) -> DocumentRecord {
        DocumentRecord {
            path: path.into(),
            root: "/workspace".into(),
            relative_path: path.trim_start_matches("/workspace/").into(),
    ...
    fn the_pane_answers_for_its_own_domain() {
        ...
        assert_eq!(pane.template_id(), DOCUMENT_VIEWER_TEMPLATE);
        assert_eq!(pane.projection_schema(), DOCUMENT_PROJECTION_SCHEMA);
    }

    #[test]
    fn identity_follows_the_path_and_survives_an_edit() {
        let before = record(
            "/workspace/docs/readme.md",
            true,
            true,
        ...
        after.byte_size = 900;
        after.modified_at = Some("2026-08-28T00:00:00Z".into());
        assert_eq!(visual_id(&before), visual_id(&after));
        ...
        assert_ne!(visual_id(&before), visual_id(&other));
        assert!(visual_id(&before).starts_with("vis_doc_"));
        assert!(visual_id(&before).len() <= 128);
    }
    ...
    fn every_unavailable_reason_is_named_rather_than_hidden() {
        ...
        assert_eq!(
            ...
            Presentability::Unavailable(UnavailableReason::NotADocument)
        );
        assert_eq!(
            presentable(&record(
                "/workspace/blob.bin",
        ...
        let mut unreadable = record("/workspace/locked.md", true, true, DocumentKind::Markdown);
        unreadable.read_error = Some("permission denied".into());
        assert_eq!(
            presentable(&unreadable),
            Presentability::Unavailable(UnavailableReason::Unreadable)
        );
        assert_eq!(
            presentable(&record(
                "/workspace/readme.md",
    ...
    #[test]
    fn every_reason_carries_a_next_step() {
        for reason in [
            UnavailableReason::Missing,
            UnavailableReason::NotText,
            UnavailableReason::NotADocument,
            UnavailableReason::NotADirectory,
            UnavailableReason::Unreadable,
            UnavailableReason::Quarantined,
            UnavailableReason::ArchiveIncomplete,
            UnavailableReason::Unsupported,
        ] {
            assert!(!reason.label().is_empty());
            assert!(!reason.remediation().is_empty(), "{reason:?}");
        }
    }

    #[test]
    fn the_binding_is_read_back_only_for_the_viewer_template() {
        let document = record(
            "/workspace/docs/readme.md",
            true,
            true,
        ...
        let request = document_viewer_create_request(&document, Some("sess_1".into()));
        ...
        assert_eq!(
            document_path_binding(&visual(DOCUMENT_VIEWER_TEMPLATE)).as_deref(),
            Some("/workspace/docs/readme.md")
        );
        assert_eq!(
            document_path_binding(&visual(super::super::TRACE_INSPECTOR_TEMPLATE)),
            None
        );
    }

    #[test]
    fn the_pane_declares_exactly_one_readable_path() {
        let document = record(
            "/workspace/docs/readme.md",
            true,
            true,
        ...
        let request = document_viewer_create_request(&document, None);
        let inputs = request.bindings.unwrap()["inputs"]
            .as_array()
            .unwrap()
            .clone();
        assert_eq!(inputs.len(), 1);
        assert_eq!(inputs[0]["kind"], WORKSPACE_FILE_BINDING_KIND);
        assert_eq!(inputs[0]["input"], DOCUMENT_INPUT);
        assert_eq!(inputs[0]["source"], "/workspace/docs/readme.md");
    }
}
