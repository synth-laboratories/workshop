...
use super::{
    document, trace_inspectability, trace_inspector_visual_id, TraceInspectability,
    DOCUMENT_PROJECTION_SCHEMA, DOCUMENT_VIEWER_TEMPLATE, TRACE_INSPECTOR_TEMPLATE,
...
impl<'a> Pane<'a> {
    ...
    pub fn presentable(self) -> Presentability {
        match self {
            Self::Trace(record) => match trace_inspectability(record) {
                TraceInspectability::Inspect => Presentability::Present,
    ...
    pub fn visual_id(self) -> String {
        match self {
            Self::Trace(record) => trace_inspector_visual_id(record),
