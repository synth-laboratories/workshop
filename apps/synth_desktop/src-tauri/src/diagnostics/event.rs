...
mod tests {
    ...
    #[test]
    fn validates_and_fills_in_derivable_fields() {
        let event = validate(input()).unwrap();
        assert_eq!(event.schema, DIAGNOSTIC_EVENT_SCHEMA);
        assert!(event.event_id.starts_with("diag_"));
        assert!(chrono::DateTime::parse_from_rfc3339(&event.timestamp).is_ok());
        assert_eq!(event.severity, Severity::Error);
    }

    #[test]
    fn rejects_labels_that_would_explode_the_index() {
        let mut unknown_component = input();
        unknown_component.component = "whatever-i-invented".into();
        assert!(validate(unknown_component).is_err());

        let mut freeform_code = input();
        freeform_code.code = "Unsupported trace projection schema".into();
        assert!(validate(freeform_code).is_err());

        let mut freeform_event = input();
        freeform_event.event = "Visual Projection Rejected".into();
        assert!(validate(freeform_event).is_err());

        let mut bad_severity = input();
        bad_severity.severity = "loud".into();
        assert!(validate(bad_severity).is_err());
    }

    #[test]
    fn bounds_message_and_identifier_length_instead_of_rejecting() {
        let mut long = input();
        long.message = "x".repeat(MAX_MESSAGE_CHARS * 3);
        long.correlation.visual_id = Some("v".repeat(MAX_IDENTIFIER_CHARS * 2));
        let event = validate(long).unwrap();
        assert_eq!(event.message.chars().count(), MAX_MESSAGE_CHARS);
        assert_eq!(
            event.correlation.visual_id.unwrap().chars().count(),
            MAX_IDENTIFIER_CHARS
        );
    }

    #[test]
    fn round_trips_through_the_journal_payload() {
    ...
    #[test]
    fn index_line_carries_correlation_and_the_idempotency_key() {
        let mut source = input();
        source.correlation.visual_id = Some("vis_1".into());
        source.correlation.rollout_id = Some("roll_1".into());
        let line = validate(source).unwrap().to_index_line(42);
        assert_eq!(line["journal_sequence"], "42");
        assert_eq!(line["visual_id"], "vis_1");
        assert_eq!(line["rollout_id"], "roll_1");
        assert_eq!(line["component"], "visual-host");
        assert!(line["_msg"].as_str().unwrap().contains("synth.trace.v5"));
    }

    #[test]
    fn every_scope_names_only_known_components() {
        for (scope, components) in SCOPES {
            for component in *components {
                assert!(is_known_component(component), "{scope} -> {component}");
            }
        }
    }

    #[test]
    fn correlation_accessors_cover_every_declared_field() {
