...
mod tests {
    ...
    #[test]
    fn ids_are_existing() {
        assert!(job_ids(&json!({"schemaVersion":SCHEMA,"evalJobIds":["job"]})).is_ok());
        assert!(job_ids(&json!({"schemaVersion":SCHEMA,"cohortId":"new"})).is_err());
    }
}
...
mod acceptance_tests {
    use super::*;
    #[test]
    fn membership_uses_current_schema_and_child_job_ids() {
        let dir = tempfile::tempdir().unwrap();
        let storage = crate::storage::Storage::open(dir.path()).unwrap();
        storage.database().with_conn(|c|{
            for (id,algorithm) in [("parent","go_ex"),("child","eval")] {
                c.execute("INSERT INTO optimizer_runs(id,algorithm_id,status,source,created_at,payload_json,updated_at) VALUES(?1,?2,'completed','test','now','{}','now')",rusqlite::params![id,algorithm])?;
            }
            c.execute("INSERT INTO optimizer_run_collection_rows(optimizer_run_id,collection,item_id,ordinal,sequence,revision,kind,details_version,details_json,updated_at) VALUES('parent','evaluations','child',0,1,1,'go_ex_child_eval','v1','{\"optimizerRunId\":\"child\"}','now')",[])?;
            c.execute("INSERT INTO optimizer_run_collection_rows(optimizer_run_id,collection,item_id,ordinal,sequence,revision,kind,details_version,details_json,score,status,updated_at) VALUES('child','evaluations','trial',0,1,1,'eval_trial','v2','{\"seed\":0,\"researchContext\":{\"taskId\":\"task\",\"model\":\"candidate\"}}',0,'failed','now')",[])?;
            let result=resolve_inputs(c,&json!({"schemaVersion":SCHEMA,"evalJobIds":["parent"],"includeChildEvals":true}))?;
            let row=&result["episodes"][0];
            assert_eq!(row["jobId"],"child");assert_eq!(row["reward"],0.0);assert_eq!(row["taskId"],"task");assert_eq!(row["model"],"candidate");
            assert_eq!(resolve_inputs(c,&json!({"schemaVersion":SCHEMA,"evalJobIds":["parent"]}))?["episodes"],json!([]));
            assert!(resolve_inputs(c,&json!({"schemaVersion":SCHEMA,"evalJobIds":["unknown"]})).is_err());
            Ok(())
        }).unwrap();
    }
    #[test]
    fn imported_standalone_job_preserves_identity_and_missing_reward() {
        let dir = tempfile::tempdir().unwrap();
        let storage = crate::storage::Storage::open(dir.path()).unwrap();
        storage.database().with_conn(|c| {
            c.execute("INSERT INTO traces(id,digest,title,source,metrics_json,metadata_json,created_at) VALUES('local','sha256:sealed','retained','import','[]','{\"runId\":\"eval-job\",\"episodeId\":\"episode\",\"effort\":\"low\",\"captureStatus\":\"partial\"}','now')", [])?;
            let result = resolve_inputs(c, &json!({"schemaVersion":SCHEMA,"evalJobIds":["eval-job"]}))?;
            assert_eq!(result["episodes"][0]["jobId"], "eval-job");
            assert_eq!(result["episodes"][0]["trialId"], "episode");
            assert_eq!(result["episodes"][0]["traceDigest"], "sha256:sealed");
            assert!(result["episodes"][0]["reward"].is_null());
            assert!(resolve_inputs(c, &json!({"schemaVersion":SCHEMA,"evalJobIds":["eval"]})).is_err());
            Ok(())
        }).unwrap();
    }
    #[test]
    fn all_450_results_page_without_loss() {
        let rows: Vec<Value> = (0..450).map(|i| json!({"trialId":i})).collect();
        let snapshot = crate::trace_query::QuerySnapshot {
            schema_version: "synth.trace-query-result.v2".into(),
            snapshot_id: "snapshot".into(),
            domain: "traces".into(),
            query_schema_version: SCHEMA.into(),
            query_ast: json!({}),
            result_ids: (0..450).map(|i| format!("r{i}")).collect(),
            result_count: 450,
        ...
        for offset in [0, 200, 400] {
            let p = page(&snapshot, offset, 200).unwrap();
            ids.extend(p["resultIds"].as_array().unwrap().clone());
            assert_eq!(p["resultDigest"], "digest");
        }
        assert_eq!(ids.len(), 450);
        assert_eq!(ids[449], "r449");
        assert!(page(&snapshot, 451, 200).is_err());
    }
}
...
mod selection_tests {
    use super::*;
    #[test]
    fn selection_deduplicates_traces_and_refuses_outside_results_without_any_plugin() {
        let row = json!({"traceDigest":"sha256:trace","traceId":"trace","containerId":"container","traceAvailability":"available","selector":{"trace_id":"trace","trace_digest":"sha256:trace","kind":"event","entity_id":"event"}});
        let snapshot = crate::trace_query::QuerySnapshot {
            schema_version: "synth.trace-query-result.v2".into(),
            snapshot_id: "snapshot".into(),
            domain: "traces".into(),
            query_schema_version: SCHEMA.into(),
            query_ast: json!({}),
            result_ids: vec!["a".into(), "b".into()],
            result_count: 2,
            facets: json!({"rows":[row,row]}),
            result_digest: "digest".into(),
            queried_at: "now".into(),
            truncated: false,
        };
        let selection = annotation_selection(&snapshot, &["a".into(), "b".into()]).unwrap();
        assert_eq!(selection["targets"].as_array().unwrap().len(), 1);
        assert_eq!(selection["targets"][0]["resultIds"], json!(["a", "b"]));
        assert_eq!(selection["startsCompute"], false);
        assert!(annotation_selection(&snapshot, &["outside".into()]).is_err());
    }
}
