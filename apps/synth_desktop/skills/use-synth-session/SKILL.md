...
| Field | Rules |
| --- | --- |
| `title` | Manual CoreRuntime rename. Calls the same `set_title(..., Manual)` path as a sidebar rename, then `thread/name/set` when the Codex attachment is live. Do not invent a second title store. Use only when the user-facing name should change. |
...
Do not call this tool on every token. Set emotion when the task mood changes, summary when the user-visible gist changes, and title only when the thread name should change.
