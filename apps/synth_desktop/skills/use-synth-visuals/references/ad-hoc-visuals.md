...
| Imported optimizer evidence | one named slot per arm, `kind: "optimizer_snapshot"`, `source: snapshot_id` | Cross-instance or portable completed-run comparisons | Export in the owning instance, import in the destination, and bind the immutable snapshot id. Prefer the returned `evidenceSummary` comparison projection; its completeness flags distinguish container-reported rollout policy cost from terminal usage-lane telemetry. Preserve source instance/run identity and digest; never read another instance's database directly. |
