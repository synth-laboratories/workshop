...
{"schemaVersion": "synth.visual-bindings.v1",
 ...
 "slots": [{"input": "stream", "slot": "stream", "kind": "live_sse", "source": "...", "poll_url": "..."}]}
```

Write `input` / `inputs`. `slot` / `slots` are a one-release copy. If both names are present and disagree, fail closed.
