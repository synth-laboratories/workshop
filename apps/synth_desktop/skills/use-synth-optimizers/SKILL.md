...
2. Choose the algorithm from the user's objective:
   ...
   - CISPO: on-policy training. Local MLX is `cispo.mlx.v1`. Hosted Tinker is `cispo.banking77.tinker.v1` and stays unavailable until the clip-identity receipt admits it. Read [references/cispo.md](references/cispo.md). PPO is not a local/hosted picker option.
...
4. Start a bounded product recipe with `start_workflow`. It returns the authoritative run, visual references, event cursor, and admission status. The host owns approval, fresh capability observation, sidecar readiness, and visual opening.
   ...
   - Local `eval.*` recipes execute through Containers and do not install or depend on the Optimizers plugin. Use `start_workflow` for an explicitly requested workspace baseline recipe. Stage candidates only for a candidate-comparison recipe that requires `candidate_set_id`; pass that returned identity to the workflow. The pinned target and recipe own the compute bounds.
...
5. Pass the workspace `recipe_id` to `start_workflow`. For candidate-comparison `eval.*` recipes, also pass the `candidate_set_id` returned by `stage_eval_candidates`. For workspace baseline evals, pass the exact healthy registered `container_id` returned by `container_list`/`container_get`, especially when multiple pools exist. `container_ensure` is for launching a workspace-declared container, not a prerequisite for evaluating an already-running registered target. Do not call `container_reconcile` merely because an externally registered target has no launch declaration. If the recipe is ready, try ordinary workflow admission; report an actual structured admission refusal instead of inventing a launch-declaration blocker. The Rust host owns commands, paths, hyperparameters, capability refresh, and credential resolution. Retrieve the winner with `get_result` — never read result files by filesystem path.
