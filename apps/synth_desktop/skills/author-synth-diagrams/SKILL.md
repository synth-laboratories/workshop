...
6. Inspect the actual rendered Desktop visual—not the JSON source. Call `capture_review` at wide and compact viewport sizes; it returns each real PNG as tool image content and its absolute `screenshot_path`. Look at both attached images, check text collisions, truncation, hierarchy, edge crossings, and focal density, then revise the same ID with `update` + new `content`. Do not shell-search for screenshots.
