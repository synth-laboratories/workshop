...
export function PaidComputeApprovalModal({ line, onApprove, onReject }: {
	...
	return <div className="paid-compute-modal-backdrop" data-testid="paid-compute-approval-modal" data-approval-digest={payload?.approvalDigest}>
		<section className="paid-compute-modal" role="dialog" aria-modal="true" aria-labelledby="paid-compute-title">
			...
			<div className="paid-compute-modal-actions">
				...
				<button ref={approveRef} type="button" className="approval-approve" onClick={() => onApprove?.(line.approvalId!, undefined, payload?.approvalDigest)}>Approve</button>
			</div>
		</section>
	</div>;
}
...
function AssistantMessageBody({ body, onOpenVisual }: { body: string; onOpenVisual: (id: string) => void }) {
	return <div className="assistant-rich-body"><Markdown source={body} renderLink={(rawTarget, children, label) => {
		const visualId = rawTarget.match(/^synth visual\s+`?([^`\s]+)`?$/i)?.[1];
		if (visualId) {
			return <button type="button" className="assistant-resource-chip assistant-visual-chip" onClick={() => onOpenVisual(visualId)} title={`Open visual ${label}`}><span aria-hidden>◈</span><strong>{children}</strong></button>;
		} else if (rawTarget.startsWith("/")) {
			const path = rawTarget.replace(/:\d+(?::\d+)?$/, "");
			return <span className="assistant-file-link"><button type="button" className="assistant-resource-chip" onClick={() => void openPath(path)} title={rawTarget}><FileTypeIcon path={path} /><strong>{children}</strong></button><button type="button" className="assistant-file-reveal" onClick={() => void revealItemInDir(path)} aria-label={`Reveal ${label} in Finder`} title="Reveal in Finder">⌗</button></span>;
		} else if (/^https?:\/\//i.test(rawTarget)) {
			return <a href={rawTarget} target="_blank" rel="noreferrer">{children}</a>;
		}
		return <>{children} ({rawTarget})</>;
	}} /></div>;
}
