...
export function ExperimentsPage({ initialId, onBack }: { initialId?: string; onBack: () => void }) {
	...
	const node = selected?.nodes.find((item) => item.id === nodeId) ?? selected?.nodes[0] ?? null;
	...
	return <section className="experiments-page detail" data-testid="experiment-detail">
		...
		<div className="lineage-workspace">
			<div className="lineage-canvas" role="listbox" aria-label="Related run lineage">
				{selected.nodes.map((item, index) => <div className="lineage-step" key={item.id}>
					<button role="option" aria-selected={node?.id === item.id} className={`lineage-node ${item.kind}`} onClick={() => setNodeId(item.id)} onKeyDown={(event) => { if (event.key === "ArrowRight") setNodeId(selected.nodes[Math.min(index + 1, selected.nodes.length - 1)]?.id ?? item.id); if (event.key === "ArrowLeft") setNodeId(selected.nodes[Math.max(index - 1, 0)]?.id ?? item.id); }}>
