...
export function TrainingWorkspace({ onStartFixture, onStartAgent, fixtureBusy = false }: { onStartFixture?: () => void; onStartAgent?: () => void; fixtureBusy?: boolean }) {
	...
	return <section className="training-workspace" aria-labelledby="training-title" data-testid="training-workspace">
		...
		<div className="training-launch-row" data-testid="optimizer-guide-sft"><div className="training-flow" aria-label="SFT workflow"><span>Collect</span><span>Train</span><span>Compare</span><span>no provider charges</span></div><div className="training-actions">{onStartAgent ? <button className="secondary-button" type="button" onClick={onStartAgent} data-testid="start-sft-agent">Plan with agent</button> : null}{onStartFixture ? <button className="secondary-button" type="button" disabled={fixtureBusy} onClick={onStartFixture} data-testid="start-sft-fixture">{fixtureBusy ? "Starting fixture…" : "Run free fixture"}</button> : null}</div></div>
		...
		{view === "setup" ? <div className="training-panel" data-testid="training-setup" data-install-state={installState}>
			...
			<Kv values={[["Platform", readiness?.platform === "apple_silicon" ? "Apple Silicon" : fixtures.readiness.platform], ["Runtime", readiness?.runtimeVersion ?? fixtures.readiness.runtime], ["Runtime health", readiness?.runtimeHealth ?? fixtures.readiness.runtimeHealth], ["Memory", fixtures.readiness.memory], ["Disk", fixtures.readiness.disk]]} />
			<div className="training-model" data-testid="on-device-training-catalog"><div className="training-section-head"><h3>{fixtures.model.name}</h3><span className="training-status" data-state={installState}>{installState === "failed" ? "Checksum failed" : installState[0].toUpperCase() + installState.slice(1)}</span></div>
				<Kv values={[["Source", installPlan?.source ?? fixtures.model.source], ["Revision", installPlan?.revision ?? fixtures.model.revision], ["Digest", installPlan?.digest ?? fixtures.model.digest], ["License", installPlan?.license ?? fixtures.model.license], ["Download", installPlan ? formatBytes(installPlan.downloadBytes) : fixtures.model.size], ["Disk after", fixtures.model.diskAfter]]} />
		...
		{confirmation ? <ConfirmationSheet kind={confirmation} artifact={artifact} onClose={() => setConfirmation(null)} onConfirm={() => void executeConfirmation(confirmation)} /> : null}
	</section>;
}
