...
export function TrainingWorkspace({ onStartAgent }: { onStartAgent?: () => void }) {
	...
	const localAvailable = recipeAvailable(trainingRecipeId(algorithm, "mlx"));
	const hostedAvailable = recipeAvailable(trainingRecipeId(algorithm, "tinker"));
	const selectedPlacementAvailable = placement === "mlx" ? localAvailable : hostedAvailable;
	const selectedRecipeReason = recipes.find((recipe) => recipe.id === trainingRecipeId(algorithm, placement))?.availabilityReason;
