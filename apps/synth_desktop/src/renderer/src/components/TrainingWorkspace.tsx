...
export function TrainingWorkspace({ onStartAgent }: { onStartAgent?: () => void }) {
	...
	return <section className="training-workspace" aria-labelledby="training-title" data-testid="training-workspace">
		<div className="training-heading"><div><span className="optimizer-eyebrow">Model training</span><h2 id="training-title">Training</h2></div><span className="training-status" data-state={readiness?.runtimeHealth === "ready" ? "ready" : "failed"}>{readiness?.runtimeHealth ?? "Unavailable"}</span></div>
