...
function recoverableMirror(record: { source: string; algorithmId: string; status: string; error?: unknown; summary?: unknown }): boolean {
    if (record.source === "hosted" && record.algorithmId === "sft" && record.status === "failed_evidence"
        && (record.summary as {trainingTransport?: string} | undefined)?.trainingTransport === "public-sft.v1") return true;
...
export function TrainingWorkspace({ onStartAgent, sessionRef, onEnsureApprovalSession }: { onStartAgent?: () => void; sessionRef?: string | null; onEnsureApprovalSession?: () => Promise<string> }) {
	...
	const [cispoUpdates, setCispoUpdates] = useState(2);
	const [cispoGroupSize, setCispoGroupSize] = useState(8);
	const [cispoCostCap, setCispoCostCap] = useState(5);
	const [cispoMaxTokens, setCispoMaxTokens] = useState(512);
	...
	const start = async () => {
		...
		try {
			...
			const record = await bridges.optimizers.startRecipe({
				...
				...(algorithm === "cispo" && placement === "tinker" ? { planOverride: { cispo: { updates: cispoUpdates, groupSize: cispoGroupSize, maxSampleTokens: cispoMaxTokens, maxCostUsd: cispoCostCap } } } : {}),
