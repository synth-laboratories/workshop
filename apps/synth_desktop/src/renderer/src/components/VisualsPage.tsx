...
export function VisualsPage({ onOpenVisual, onGoToChat, onBack, onCreate }: Props) {
	...
	return (
		<section className={`visuals-page${focusVisualId ? " visuals-page-focus" : ""}`} data-testid="visuals-page">
			...
			<div className={`visuals-layout${focusVisualId ? " focus" : ""}`} style={focusVisualId ? undefined : { gridTemplateColumns: `${listWidth}px 8px minmax(360px, 1fr)` }}>
				...
				{selected && !focusVisualId ? <PaneResizeHandle value={listWidth} onChange={updateListWidth} minPrimary={280} minSecondary={360} ariaLabel="Resize visual list and preview" direction="primary" /> : null}
				{selected ? (
