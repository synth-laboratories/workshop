...
import { formatNodeFailureReason } from "../runtime/experimentPresentation";
			...
			<div className="lineage-workspace-main">
				{group ? (
					<header className="lineage-workspace-header">
						<div style={{ minWidth: 0, flex: 1 }}>
							...
							<h2>{group.title}</h2>
							<p data-testid="experiment-header-task" title={group.task ?? undefined} style={{ overflowWrap: "anywhere" }}>
								{missing(group.task)}
							</p>
							<p data-testid="experiment-header-provenance">
								Model {missing(group.model)} · updated {new Date(group.updatedAt).toLocaleString()}
							</p>
						</div>
				...
				<LineageCanvas
					nodes={lineageNodes.map((item) => ({
						...
						title: item.title,
						status: item.status,
						reason: formatNodeFailureReason(item),
					}))}
					edges={edges}
