...
/** Rank then column: Home is first, End is last. */
export function rankedOrder(nodes: RankedNode[]): RankedNode[] {
	return [...nodes].sort((a, b) => a.rank - b.rank || a.column - b.column);
}

export function optionDomId(id: string): string {
	return `lineage-option-${id}`;
}

/** Scale + translate so every ranked node sits inside the canvas client box. */
export function fitRankedToViewport(
	nodes: RankedNode[],
	viewport: { width: number; height: number },
	options?: { padding?: number; scale?: number; maxScale?: number },
): ViewTransform {
	const padding = options?.padding ?? VIEW_PADDING;
	const maxScale = options?.maxScale ?? 1.5;
	if (!nodes.length || viewport.width <= 0 || viewport.height <= 0) {
		return { x: 0, y: 0, scale: options?.scale ?? 1 };
	}
	...
	const scale = options?.scale ?? Math.min(maxScale, innerWidth / graphWidth, innerHeight / graphHeight);
	return {
		x: padding + (innerWidth - graphWidth * scale) / 2 - minX * scale,
		y: padding + (innerHeight - graphHeight * scale) / 2 - minY * scale,
		scale,
	};
}
