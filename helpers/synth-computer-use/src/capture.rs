...
mod tests {
    ...
    /// A pid with no windows must not be an error: menu-bar apps and freshly
    /// launched apps are both normal.
    #[cfg(target_os = "macos")]
    #[test]
    fn a_process_with_no_windows_yields_no_window_ids() {
        // pid 0 is the kernel and never owns a window.
        assert!(window_ids(0).is_empty());
    }
}
