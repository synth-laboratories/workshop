...
mod tests {
    ...
    #[test]
    fn the_requirement_pins_identifier_and_team_not_just_the_apple_anchor() {
        let requirement = default_parent_requirement("ABCDE12345");
        assert!(requirement.contains("anchor apple generic"));
        // Without the identifier and team, every Developer ID app on the
        // machine would satisfy this.
        assert!(requirement.contains("identifier \"ai.usesynth.workshop\""));
        assert!(requirement.contains("subject.OU] = \"ABCDE12345\""));
    }

    #[test]
    fn a_missing_launch_nonce_is_refused_when_one_is_required() {
        std::env::remove_var(LAUNCH_NONCE_ENV);
        assert!(verify_launch_nonce(true).is_err());
        assert!(verify_launch_nonce(false).is_ok());
        std::env::set_var(LAUNCH_NONCE_ENV, "abc123");
        assert!(verify_launch_nonce(true).is_ok());
        std::env::remove_var(LAUNCH_NONCE_ENV);
    }

    /// The parent of the test runner is a shell, not a signed Synth Desktop, so
    /// this must fail. A helper that authenticated whatever launched it would
    /// be a helper with no access control at all.
    #[cfg(target_os = "macos")]
    #[test]
    fn an_unsigned_parent_does_not_satisfy_the_requirement() {
        let error = verify_parent(&default_parent_requirement("ABCDE12345"))
            .unwrap_err()
            .to_string();
        assert!(
            error.contains("not authorized") || error.contains("could not inspect"),
            "{error}"
        );
    }
}
