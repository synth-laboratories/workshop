...
**Target:** public v0.1 release candidate  
...
- First run exposes **Continue locally** and **Sign in to Synth**. The setup-agent modal/card is intentionally absent.
- The v0.1 launch picker exposes local **Laguna XS 2.1**, OpenRouter **GPT 5.6 Luna**, OpenRouter **Laguna S 2.1**, and Synth Cloud **Laguna S 2.1**. Intern targets remain only in dormant catalog/fixture code and are filtered out by `LAUNCH_PICKER_TARGETS`.
- The sidebar exposes Chats, Connectors, Research → Visuals/Optimizers, Inventory → Containers · Traces · Usage, and Settings. There is no v0.1 Intern navigation.
- `ContainerPane` is the container inspector. Visual artifacts and Trace V5 share `VisualPane`; the app does not have three peer Visuals/Containers/Traces tabs.
- Device pairing keeps credentials in Rust custody and writes the v0.1 key to a desktop-managed `0600` env file. OS-keychain storage is the documented OAuth v1 target.
- The deterministic renderer suite currently has 13 Playwright spec files and no expected-fail/skip/fixme/todo escape markers. This is coverage inventory, not a green release claim.
...
3. **Container inspector:** executable environments, tasks, datasets/evals, rollouts, health, and live state can be opened beside the current agent session or from Inventory.
4. **Shared visual/trace inspector:** visual artifacts and imported/stored traces open in the same `VisualPane`; Trace V5 uses its rollout inspector rather than a separate peer trace panel.
5. **Research routes:** Visuals and Optimizers are first-class research pages; Inventory is the management/recovery route for Containers, Traces, and Usage.
...
- First-class adjacent evidence surfaces: a Container inspector plus a shared visual/trace inspector, with open/close/expand/reopen behavior and correct binding to active work. Inventory provides the resizable management layout.
...
- Any feature that depends on an unstable backend contract or cannot complete the launch acceptance matrix for the candidate.
...
### [alpha] but remain candidate scope when proven
...
- **Three first-class research primitives, two inspector implementations.** Visuals, Containers, and Traces remain product primitives. Containers use `ContainerPane`; visuals and traces share `VisualPane`. The UX and tests must describe this actual architecture rather than promise three peer tabs that do not exist.
...
- [ ] Key entry is masked, held only by the Rust host, stored in the desktop-managed `0600` credential env file for v0.1, never logged, and has validate/replace/revoke guidance. OS-keychain storage remains the OAuth v1 target, not a v0.1 claim.
...
4. The first-run choice presents equal **Continue locally** and **Sign in to Synth** actions. There is no setup-agent modal, wizard, or setup card.
5. Continuing locally records the choice and opens the normal landing/composer with the launch model picker; signing in opens Settings → Account and starts browser pairing only when the user asks.
6. The first prompt is entered in the normal composer and counts as successful only after a real provider response in a durable conversation.
7. The user can switch providers from the launch picker and reopen that conversation after app restart.
...
- [ ] Account pairing and local model management remain reachable from Settings; no removed setup wizard is referenced as resumable.
...
The default screen is an agent workspace, not a dashboard. The current implementation has two adjacent inspector types, not three peer panel modes:
...
- Visual artifacts and traces open in the shared `VisualPane`; containers open in `ContainerPane`. An optional inference rail may also appear, but is not one of the three research primitives.
- Opening or switching an inspector never destroys agent state, scroll position, current selection, or an in-progress run.
- Both inspectors can close and expand/collapse; the Inventory split exposes persisted drag resizing. Any broader resize or deep-link behavior remains a requirement until demonstrated on the exact candidate.
- The active inspector object clearly identifies the agent session, run, task, or artifact to which it is bound.
...
### Shared Visuals/Traces inspector contract
...
- Open imported/stored Trace V5 records through `trace.rollout_inspector.v1` in the same pane, while preserving trace identity, schema, hashes, and missing/corrupt-evidence states.
...
### Trace behavior within the shared inspector
...
- **Chats:** normal durable conversations and agent sessions occupy the main sidebar list.
- **Connectors:** searchable MCP connector catalog.
- **Research → Visuals:** reusable views bound to real data.
- **Research → Optimizers:** optimizer runs, imports, recipes, and their visual artifacts.
- **Inventory → Containers · Traces · Usage:** one secondary management/recovery route; models are managed under Settings, not Inventory.
...
- Treat the shipped renderer and its tests as the current visual contract: [`app.css`](apps/synth_desktop/src/renderer/src/styles/app.css), [`visual-alignment.spec.ts`](apps/synth_desktop/tests/bombadil/visual-alignment.spec.ts), [`layout-invariants.spec.ts`](apps/synth_desktop/tests/playwright/layout-invariants.spec.ts), and [`poolside-polish.spec.ts`](apps/synth_desktop/tests/playwright/poolside-polish.spec.ts). The previously referenced style-guide Markdown files are not present in this checkout and must not be cited as authoritative.
...
- [ ] Synth credentials stay in Rust custody and the v0.1 desktop-managed `0600` env file; redacted from UI logs, traces, exports, screenshots, analytics, and crash reports. Do not claim OS-keychain storage until the OAuth v1 path ships.
...
- [ ] Website/app funnel events cover landing CTA, signup, download, pair, local-vs-sign-in first-run choice, first real response, first eval, and failure category.
...
3. First run: download, choose Continue locally or Sign in to Synth, then choose a launch model and send the first prompt.
...
- [ ] 60–90 s first-run: site/signup/download, Continue locally or browser pairing, model selection, first prompt; do not show a removed setup modal.
...
- [x] Add the v0.1 gate harness under `/Users/joshuapurtell/Documents/GitHub/evals/workshop`, covering readiness, provider parity, Craftax/live visual contracts, receipts, security controls, and deferred-Intern absence. Auth breadth, Trace V5 correlation, optimizer evidence, and native CUA remain incomplete gates rather than implied coverage.
...
### Required release-candidate acceptance sequence
...
5. [ ] During an active agent task, open/close/expand/reopen the Container inspector and shared visual/trace inspector; exercise Inventory resizing without losing state or obscuring the composer.
6. [ ] Attach Craftax from Inventory/Containers; let the agent launch one real rollout; watch the real frame in the visual inspector; open the correlated Trace V5 view in that same inspector.
...
- [ ] Known issues triaged: fix, remove the surface, or explicitly isolate it outside supported v0.1. Launch gates permit no expected-fail/skip waiver.
...
- CUA screenshots/video for each required release-candidate journey.
...
These are code-audited facts, not blanket launch approval:

- The renderer has 13 Playwright spec files covering first run/account, lifecycle, runtime regressions, layout, navigation, provider behavior, visuals, optimizers, slash/voice, and launch-debt locks. Test names or old pass counts are not release evidence.
- Bombadil has layout, visual-alignment, composer-surfaces, and launch-debt campaigns; the gate runs the configured suite but cannot enforce a pass-count floor because Bombadil does not report one.
- The eval gate implements independently graded provider-parity tasks, real Craftax catalog/two-rollout checks, live visual creation, redaction, evidence hashing, artifact checks, source cleanliness, and 10,000-transition state fuzzing.
- The live Trace V5 correlation check remains an intentional hardcoded blocker. Auth error breadth, SSE/fault campaigns, signed-artifact qualification, performance/memory evidence, website funnel evidence, and the 37-item manual receipt are not complete.
- The Workshop and evals working trees were dirty during this audit. `gate:release` is designed to reject that state; only a fresh run against committed revisions and the exact artifact can change the launch verdict.
