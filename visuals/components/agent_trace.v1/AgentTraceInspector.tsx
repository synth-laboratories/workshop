...
const score = (value: unknown) => typeof value === "number" && Number.isFinite(value) ? new Intl.NumberFormat("en-US", { maximumFractionDigits: 4 }).format(value) : value == null ? "Unavailable" : String(value);
...
export function TraceEventCard({ item, selected, onSelect, sourceLink }: {
  ...
  return <article className={`ati-event ${selected ? 'ati-selected' : ''}`} data-testid={`agent-trace-event-${item.item_id}`}>
    ...
    {family === 'reward' && <><p className="ati-reward">{score(d.value)} {d.units ?? 'reward'}{d.cumulative != null && <small> · cumulative {score(d.cumulative)}</small>}</p><p>{d.provenance}</p><Field label="Reward components" value={d.components}/></>}
...
export function AgentTraceInspector({ projection, actorId, onActorChange, cursorMs, onSelectEvent, renderEnvironment, sourceUrl, annotations, onAnnotate, onOpenAnnotation, extensions = [], initialView = 'general', selection }: {
  ...
  const [localActor, setActor] = useState('all'); const actor = actorId ?? localActor;
  const [section, setSection] = useState<TraceSection>('rollout'); const [query, setQuery] = useState('');
  const [selectedId, select] = useState<string | null>(null); const [pin, setPin] = useState('');
  const [limit, setLimit] = useState(60);
  const [view, setView] = useState<string>(initialView);
  const identity = projection.trace_id ?? projection.capture_id;
  useEffect(() => { select(null); setLimit(60); setPin(''); setActor('all'); }, [identity]);
  useEffect(() => { setLimit(60); }, [actor, section, query]);
  ...
  return <section className="agent-trace-inspector" data-testid="agent-trace-inspector">
    ...
    <div className="ati-toolbar"><label>Agent <select aria-label="Trace agent" value={actor} onChange={e => { changeActor(e.target.value); }}><option value="all">All agents</option>{lanes.map(l => <option key={l.lane_id} value={l.actor_id ?? l.lane_id}>{l.display_name ?? l.lane_id} {l.role ? `· ${l.role}` : ''}</option>)}</select></label>
      <label>Compare <select aria-label="Compare agent" disabled={actor === "all"} title={actor === "all" ? "Select one agent before comparing" : "Compare with another agent"} value={pin} onChange={e => setPin(e.target.value)}><option value="">None</option>{lanes.filter(l => l.actor_id !== actor).map(l => <option key={l.lane_id} value={l.actor_id ?? l.lane_id}>{l.display_name ?? l.lane_id}</option>)}</select></label>
