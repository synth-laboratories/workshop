...
test('inspector requests pinned bounded windows and rejects identity drift',async()=>{
 ...
 function Parent(){const [version,setVersion]=React.useState(0);const [snapshot,setSnapshot]=React.useState('sha256:snapshot');window.rerender=()=>setVersion(v=>v+1);window.changeSnapshot=()=>setSnapshot('sha256:new-snapshot');const initial=packet(0);initial.view_window.snapshotDigest=snapshot;return <Shell data={initial} traceResearch={client}/>;}
 createRoot(document.getElementById('root')).render(<Parent/>);`,resolveDir:root,loader:'tsx'},bundle:true,write:false,format:'iife',platform:'browser',jsx:'automatic',outfile:'window.js'});
 const browser=await chromium.launch();try {
  ...
  assert.match(await page.getByLabel('Retained trace window').innerText(),/Showing events 201–400 of 402/);
  ...
  await page.evaluate(()=>window.changeSnapshot());await page.getByText(/Showing events 1–200 of 402/).waitFor();
