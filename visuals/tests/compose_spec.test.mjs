...
test("compose.visual.v1 advertises components on the template, not as templates", () => {
  ...
  const declared = template.inputs ?? template.slots;
  ...
  assert.deepEqual(declared[2].accepts, ["optimizer_run", "fixture", "inline"]);
  assert.deepEqual(
...
test("optimizer_run example binds the optimizer dialect, not stream", () => {
  ...
  assert.deepEqual(
    (example.inputs ?? example.slots).find((row) => (row.input ?? row.slot) === "optimizer_run").data.events.map((event) => event.type),
