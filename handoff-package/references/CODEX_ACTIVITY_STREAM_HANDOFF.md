...
**Grading isolation is about authority, not visibility.** It is enforced by the Evals-owned Harbor runner and private trace-review authority—not by blinding traces. So there is **no held-out reason** to withhold a live stream from Evals. Constraints that do apply: keep credential redaction, keep the contamination gate's premise that one capture = one actor's calls, and never let the live channel become terminal authority for a verdict. Exact private implementation paths belong in the Evals repository.
