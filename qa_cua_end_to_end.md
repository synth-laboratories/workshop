...
Codex agent
  ...
  ├─ Containers: executable environment and rollout state
  ├─ Visuals: live and completed interpretation
  └─ Traces: evidence and provenance
```

QA should spend depth here before broadening feature coverage. A failure in provider parity, agent lifecycle, or the Visuals/Containers/Traces side-panel relationship is a launch blocker even if secondary pages pass.
...
- **XFAIL:** known debt with a deterministic expected-failing test, owner, and rationale; never permitted for security, corruption, first-run, or deceptive success.
...
Run from `/Users/joshuapurtell/Documents/GitHub/workshop` on the exact release revision and retain complete output:

```bash
npm test
npm --prefix apps/synth_desktop run typecheck
npm --prefix apps/synth_desktop run test:playwright
npm --prefix apps/synth_desktop run test:bombadil
npm --prefix apps/synth_desktop run rust:test
npm --prefix apps/synth_desktop run frontend:build
npm --prefix apps/synth_desktop run dev:verify
```

Also execute the Laguna, local-runtime, visual registry, container/Craftax, and eval-suite commands documented by their pinned revisions. Record exact commands here before sign-off; do not summarize “tests passed” without logs.
...
- Choose Local Laguna and Synth API independently.
- Skip/resume setup; edit starter prompt; stop generation; retry.
- Assert setup completion only after a real provider response and durable conversation restore after relaunch.
...
- Start a real Codex task, then open Visuals, Containers, and Traces in turn while output is streaming and after completion.
- Resize, collapse, reopen, switch rapidly, select an object, navigate to related evidence, restart the app, and restore the session.
- Assert each panel stays bound to the correct session/run/object; selection and transcript state survive; no panel covers the composer or steals focus.
...
- Empty/loading/populated/error states; back/forward/restart; deep link/open relationships.
...
- [ ] Keep deterministic provider/bridge fixtures for every state and error in Section 2.
- [ ] Extend session lifecycle tests for kill/reload, queue, partial stream, provider provenance, retry idempotency.
- [ ] Extend runtime regressions for composer/inspector separation, terminal/output, reconnect, and stale authority.
- [ ] Extend sidebar/navigation/search tests across empty/many/long/error states.
- [ ] Add onboarding/auth/download-return fixture coverage without storing real credentials.
- [ ] Add local model setup/download/load/unload state-machine coverage.
- [ ] Add Synth Cloud credential/provider/error matrix.
- [ ] Add container catalog, preflight, SSE resume, live counts/frame binding, cancel/retry.
- [ ] Add trace import/open/corruption and visual template lifecycle.
- [ ] Add GEPA lineage/diff/comparison, alpha gating for GELO/SFT, and absence assertions for deferred Intern.
- [ ] Add keyboard/focus/reduced-motion/text-expansion journeys.
- [ ] Assert user-observable outcomes and authoritative state, not implementation class names alone.
...
- [ ] Visual alignment for landing/setup, settings, Inventory, live eval, trace, optimizer, error/empty/loading states.
...
4. Install/open app; pair account.
5. Choose Synth API; send real prompt; stop/retry; open usage.
6. Restart app; search and reopen conversation.
7. Export redacted evidence.

### CUA-B — Poolside-like local Laguna path

1. Fresh app/model state; open local setup.
...
2. Open Containers beside the agent; register/discover Craftax and inspect catalog/held-out seeds.
3. Have the agent configure a small eval with explicit model/prompt/harness/concurrency/budget; review and approve.
4. Pass preflight and launch; switch to Visuals and watch actual frames plus aggregate progress without losing the agent conversation.
5. Interrupt/reconnect the live stream; inspect truthful reconnect/staleness state.
6. Switch to Traces; correlate an environment observation, agent action, reward, frame, and model/tool event.
...
- Every finding gets fix + regression, removal, or explicit XFAIL/owner; screenshots go in the evidence packet.
