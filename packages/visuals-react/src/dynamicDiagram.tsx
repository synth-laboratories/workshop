...
import {useVisualPlaybackDriver} from "./playback.ts";
export type DynamicDiagramServices={content:()=>Promise<{base64:string}|null|undefined>;rendition:(theme:"dark"|"light")=>Promise<{mediaType:string;base64:string}|null|undefined>;render:()=>Promise<unknown>};
const publicError=(error:unknown)=>error instanceof Error?error.message:String(error);
...
export function DynamicDiagramSurface({id,title,revision,services}:{id:string;title:string;revision:number|string;services:DynamicDiagramServices}) {
	const visualId=id;
	const [source, setSource] = useState<string | null>(null);
	const [posterUrl, setPosterUrl] = useState<string | null>(null);
	const [error, setError] = useState<string | null>(null);
	const [showSource, setShowSource] = useVisualState("dynamic.source",false);
  const [playing, setPlaying] = useVisualState("dynamic.playing",false);
  const [seekTime,setSeekTime]=useVisualState<number|null>("dynamic.timeMs",null,{type:"number",minimum:0,clock:"diagram.milliseconds"});
  const setTimeMs=(value:number)=>setSeekTime(value);
  const [reduceMotion, setReduceMotion] = useVisualState("dynamic.reduceMotion",()=>window.matchMedia("(prefers-reduced-motion: reduce)").matches);
	const [notice, setNotice] = useState<string | null>(null);
	const [reload, setReload] = useState(0);
	const retryToken = useRef("");
	retryToken.current = `${visualId}:${String(revision)}`;
  ...
  const timeMs=seekTime ?? scene?.posterTimeMs ?? 0;
	...
	useEffect(() => {
		...
		void (async () => {
			...
			try {
				const asset = await services.content();
			...
			try {
				const rendition = await services.rendition(theme);
	...
	}, [visualId, revision, services, reload]);

  useVisualPlaybackDriver({
    clock:"diagram.milliseconds",playingControl:"dynamic.playing",playing:playing && Boolean(scene),intervalMs:50,
    project:(values):Record<string,number|boolean>=>{
      if(!scene)return {};
      const next=values["dynamic.reduceMotion"]===true
        ? scene.reducedMotion==="final"?scene.durationMs:scene.posterTimeMs
        : Math.min(scene.durationMs,Number(values["dynamic.timeMs"] ?? scene.posterTimeMs)+50);
      return {"dynamic.timeMs":next,"dynamic.playing":values["dynamic.reduceMotion"]!==true && next<scene.durationMs};
    },
    fallback:()=>{if(scene){const next=Math.min(scene.durationMs,timeMs+50);setTimeMs(next);if(next>=scene.durationMs)setPlaying(false);}},
  });
	...
	const seekBeat = (index: number) => { const beat = scene?.beats[index]; if (beat) { setPlaying(false); setTimeMs(beat.atMs);setSeekTime(beat.atMs); } };
	const exportStill = () => {
		if (!posterUrl) return;
		try { const link = document.createElement("a"); link.href = posterUrl; link.download = `${title || "systems-explainer"}-still.svg`; link.click(); setNotice(`Exported ${link.download}`); }
		...
		<div className="systems-dynamic" data-testid="visual-systems-dynamic" data-theme={scene?.theme === "technical-dark" ? "dark" : "light"}>
			<div className="systems-dynamic-toolbar">
				...
				<div className="systems-visual-actions">
					<button type="button" disabled={!scene} onClick={() => { if (reduceMotion && scene) setTimeMs(scene.reducedMotion === "final" ? scene.durationMs : scene.posterTimeMs); else {if(playing)setSeekTime(timeMs);setPlaying((value) => !value);} }}>{playing ? "Pause" : "Play"}</button>
					...
					<button type="button" onClick={() => { const token = retryToken.current; void (async () => { await services.render(); if (retryToken.current === token) setReload((value) => value + 1); })(); }}>Retry</button>
				</div>
			</div>
			...
			{showSource ? <pre className="systems-visual-source" data-testid="visual-systems-dynamic-source">{source ?? error ?? "Source unavailable."}</pre> : scene ? (
				<>
					<div className="systems-dynamic-stage">
						<svg viewBox={`0 0 ${scene.canvas.width} ${scene.canvas.height}`} role="img" aria-label={title}>
							<defs><marker id={`arrow-${id}`} viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" /></marker></defs>
					...
					<div className="systems-dynamic-timeline">
						<input type="range" min="0" max={scene.durationMs} step="10" value={timeMs} aria-label="Explainer timeline" onChange={(event) => { setPlaying(false); setTimeMs(Number(event.currentTarget.value));setSeekTime(Number(event.currentTarget.value)); }}/>
			...
			) : posterUrl ? <div className="systems-dynamic-stage"><img src={posterUrl} alt={title} /></div> : <p className="visual-loading">{parsedScene.error ?? error ?? "Loading systems explainer…"}</p>}
		</div>
	);
}
