...
import { pageTrailSchema } from "@synth/workshop-visuals/runtime/presentationSchemas.ts";
...
export function CollectionBrowser({
  ...
  const [cursorTrail, setCursorTrail] = useVisualState<Array<string | null>>(`collection.${collection}.pages`, [null], pageTrailSchema);
  const [active, setActive] = useVisualState(`collection.${collection}.active`, false);
  const [state, setState] = useState<PageState>({ status: "idle", page: null });
  const [selected, setSelected] = useVisualState<string | null>(`collection.${collection}.selected`, null);
  const [detail, setDetail] = useState<{ itemId: string; row: RunCollectionRowLike | null; error?: string } | null>(null);
  const key = filterKey(filter);
  const previousFilter = useRef(`${collection}:${key}:${descending}`);
  ...
  useEffect(() => {
    const next = `${collection}:${key}:${descending}`;
