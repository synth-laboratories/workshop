...
import { pageMapSchema } from "@synth/workshop-visuals/runtime/presentationSchemas.ts";
...
export function RolloutBrowser({
  ...
  const [filter, setFilter] = useVisualState<Filter>("rollouts.filter", "all");
  const [search, setSearch] = useVisualState("rollouts.search", "");
  const [openGroup, setOpenGroup] = useVisualState<string | null>("rollouts.openGroup", null);
  const [pageByGroup, setPageByGroup] = useVisualState<Record<string, number>>("rollouts.pageByGroup", {}, pageMapSchema);
  const [localInspectedId, setLocalInspectedId] = useVisualState<string | null>("rollouts.localInspectedId", null);
