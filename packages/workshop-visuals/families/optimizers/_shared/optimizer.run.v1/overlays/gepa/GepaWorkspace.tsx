...
export function GepaWorkspace({
  ...
  const [presentationState, setPresentationState] = useVisualState<GepaPresentationState>("gepa.presentationState", DEFAULT_GEPA_PRESENTATION_STATE);
  ...
  useEffect(() => {
    if (visualSession) { setHydratedRunId(run.id); return; }
  ...
  }, [run.id, visualSession]);

  useEffect(() => {
    if (visualSession || hydratedRunId !== run.id) return;
    saveGepaPresentationState(run.id, presentationState, typeof window === "undefined" ? undefined : window.localStorage);
  }, [hydratedRunId, presentationState, run.id, visualSession]);
