...
import { composeCursorSchema, resolveEventCursor } from "../../../runtime/presentationSchemas.ts";
...
export function Shell(props: ShellProps) {
  ...
  const [cursor, setCursor] = useVisualState<Cursor | null>("compose.cursor", null, composeCursorSchema);
  ...
  function selectCursor(
    placementId: string,
    _event: LiveEvalEvent,
    identity: string,
    _sequence?: number | string | null
  ) {
    setCursor({
      placementId,
      identity,
  ...
  return (
    ...
    >
      {parsed.spec.placements.map((placement) => {
        ...
        const cursorPlacement = parsed.spec.placements.find((item) => item.id === cursor?.placementId);
        return (
          <DetailModal
            key={placement.id}
            event={cursor && cursorPlacement && cursor.placementId === placement.from
              ? resolveEventCursor(sourceFor(cursorPlacement).events, cursor.identity)
              : null}
