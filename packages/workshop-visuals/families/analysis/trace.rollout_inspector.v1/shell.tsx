...
export function Shell({ title, lede, projection, data, analysisFindings = [], traceResearch }: ShellProps) {
  ...
  const [tab, setTab] = useVisualState<"trace" | "evidence" | "metadata">("inspector.tab", "trace", {"options":["trace","evidence","metadata"]});
  ...
  return <VisualChrome kicker="Trace V5 · sealed" title={title ?? payload.trace_id}
    ...
    {tab === "trace" ? <AgentTraceInspector key={`${payload.trace_digest}:${window?.offset ?? 0}`} initialView="rollout" extensions={[{id: "rollout", label: "Rollout", base: "general", renderItem: () => null, filterItems: rows => rows.filter(item => !item.kind.startsWith("model_call."))}]} projection={{...visual!, trace_id: payload.trace_id, losses: visual?.losses?.map(String), items: items.map(item => ({
      ...item,
      title: item.title ?? (item.kind.startsWith("model_call.") ? primary(item) || undefined : undefined),
      detail: {...item.detail, ...(native(item).text || native(item).command ? {
        content: [native(item).text, native(item).command, native(item).aggregated_output].filter(value => typeof value === "string").join("\n")
      } : {})}
    }))}} annotations={payload.annotation_view ? [...new Map([...traceAnnotations(items as any), ...payload.annotation_view.records].map(note => [note.id, note])).values()] : undefined} /> : null}
