...
import { retainedStreamInput } from "../../../runtime/retainedStreamInput.ts";
...
function asStream(raw: unknown): StreamPayload {
  return retainedStreamInput(raw) as StreamPayload;
}
...
export function Shell(props: ShellProps) {
  ...
  const [chosenLane, setChosenLane] = useVisualState<string | null>("craftax.chosenLane", scope?.selection?.initial_rollout_id ?? null);
  const [evaluationCutoff, setEvaluationCutoff] = useVisualState<number | null>("craftax.evaluationCutoff", null, {"type":"number"});
  const [laneCutoff, setLaneCutoff] = useVisualState<number | null>("craftax.laneCutoff", null, {"type":"number"});
  const [playing, setPlaying] = useVisualState("craftax.playing", false);
  const [speed, setSpeed] = useVisualState("craftax.speed", 1);
  const [selectedTraceId, setSelectedTraceId] = useVisualState<string | null>("craftax.selectedTraceId", null);
  const [traceMode, setTraceMode] = useVisualState<"focus" | "full">("craftax.traceMode", "full", {"options":["focus","full"]});
  const [surface, setSurface] = useVisualState<"replay" | "transcript" | "raw" | "metrics" | "integrity">("craftax.surface", "replay", {"options":["replay","transcript","raw","metrics","integrity"]});
  const [selectedCallId, setSelectedCallId] = useVisualState<string | null>("craftax.selectedCallId", null);
  const [transcriptMode, setTranscriptMode] = useVisualState<"focus" | "full">("craftax.transcriptMode", "full", {"options":["focus","full"]});
  const [showEarlierGroups, setShowEarlierGroups] = useVisualState("craftax.showEarlierGroups", false);
  const [framePlaying, setFramePlaying] = useVisualState("craftax.framePlaying", false);
  const [frameFps, setFrameFps] = useVisualState("craftax.frameFps", 4);
  ...
  useVisualPlaybackDriver({
    clock:"craftax.moments",playingControl:"craftax.playing",
    playing:playing && moments.length>0,intervalMs:700/speed,
    project:values=>{
      const next=moments.find(index=>index>Number(values["craftax.evaluationCutoff"] ?? -1));
      const last=next==null || next>=(moments.at(-1) ?? 0);
      return {"craftax.evaluationCutoff":next ?? moments.at(-1) ?? null,"craftax.playing":!last};
    },
    fallback:()=>{
      const next=moments.find(index=>index>(evaluationCutoff ?? -1));
      setEvaluationCutoff(next ?? moments.at(-1) ?? null);
      if(next==null || next>=(moments.at(-1) ?? 0))setPlaying(false);
    },
  });
  ...
  const nextFrameCutoff=(cutoff:number|null)=>{
    const current=laneEvents[Math.min(cutoff ?? laneEvents.length-1,laneEvents.length-1)] ?? frameEvents[0];
    const sequence=craftaxEventSequence(current,-1);
    const index=frameEvents.findLastIndex(event=>craftaxEventSequence(event,-1)<=sequence);
    return laneEvents.indexOf(frameEvents[(index+1)%frameEvents.length]);
  };
  useVisualPlaybackDriver({
    clock:"craftax.frames",playingControl:"craftax.framePlaying",
    playing:framePlaying && frameEvents.length>0,intervalMs:1000/frameFps,
    project:values=>{
      const cutoff=values["craftax.laneCutoff"];
      const next=nextFrameCutoff(typeof cutoff==="number"?cutoff:null);
      return next>=0?{"craftax.laneCutoff":next}:{} as Record<string,number>;
    },
    fallback:()=>{const next=nextFrameCutoff(laneCutoff);if(next>=0)setLaneCutoff(next);},
  });
  ...
  return (
	...
	>
      ...
      <section className="cv-workspace cv-surface-replay" data-visual-landmark="primary-surface">
        ...
        <aside className="cv-panel cv-details" data-testid="craftax-frame-call-panel" aria-live="polite" tabIndex={0} aria-label="Selected frame and model call details">
