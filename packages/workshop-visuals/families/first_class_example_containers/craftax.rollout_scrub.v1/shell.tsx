...
export function Shell(props: ShellProps) {
  ...
  const [index, setIndex] = useVisualState("craftax.rollout.index", 0);
  const [playing, setPlaying] = useVisualState("craftax.rollout.playing", false);
  const step = steps[Math.min(index, Math.max(steps.length - 1, 0))] ?? steps[0];

  useVisualPlaybackDriver({
    clock:"craftax.rollout.steps", playingControl:"craftax.rollout.playing",
    playing:playing && steps.length > 1, intervalMs:450,
    project:values=>({"craftax.rollout.index":(Number(values["craftax.rollout.index"])+1)%steps.length}),
