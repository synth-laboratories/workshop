...
function LaneReplay({ laneEvents, streamBase }: { laneEvents: LiveEvalEvent[]; streamBase: URL | null }) {
  const [cursor, setCursor] = useVisualState<number | null>(`container.lane.${project(laneEvents)[0]?.name ?? "unknown"}.cursor`, null, {type:"number"});
...
export function Shell(props: ShellProps) {
  ...
  const [globalCursor, setGlobalCursor] = useVisualState<number | null>("container.cursor", null, {type:"number"});
