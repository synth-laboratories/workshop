...
export function TraceWorkbench({ branding, ...props }: TraceWorkbenchProps & { branding: TraceWorkbenchBranding }) {
  ...
  const [selectedTrialId, setSelectedTrialId] = useVisualState<string | null>("trace.selectedTrialId", null);
  const [selectedCall, setSelectedCall] = useVisualState("trace.selectedCall", 0);
  const [selectedFrame, setSelectedFrame] = useVisualState<number | null>("trace.selectedFrame", null, {"type":"number"});
  const [following, setFollowing] = useVisualState("trace.following", true);
  const [playing, setPlaying] = useVisualState("trace.playing", false);
  const [query, setQuery] = useVisualState("trace.query", "");
  const [loaded, setLoaded] = useState<LoadedMedia | undefined>(undefined);
  const [aggregateFilter, setAggregateFilter] = useVisualState<AggregateFilter>("trace.aggregateFilter", null, {type:"object",nullable:true,oneOf:[
    {type:"object",required:["kind","name"],additionalProperties:false,properties:{kind:{type:"string",options:["achievement"]},name:{type:"string"}}},
    {type:"object",required:["kind","low","high","inclusiveHigh"],additionalProperties:false,properties:{kind:{type:"string",options:["reward"]},low:{type:"number"},high:{type:"number"},inclusiveHigh:{type:"boolean"}}},
  ]});
  ...
  useVisualPlaybackDriver({
    clock:"trace.frames",playingControl:"trace.playing",
    playing:playing && Boolean(view?.frames.length),intervalMs:450,
    project:(values):Record<string,number|boolean>=>{
      const next=Number(values["trace.selectedFrame"] ?? 0)+1;
      if(!view || next>=view.frames.length)return {"trace.playing":false};
      const owner=view.steps.findIndex(candidate=>candidate.frames.includes(next));
      return {"trace.selectedFrame":next,...(owner>=0?{"trace.selectedCall":owner}:{})};
    },
    fallback:()=>{
      const next=(selectedFrame ?? 0)+1;
      if(!view || next>=view.frames.length){setPlaying(false);return;}
      const owner=view.steps.findIndex(candidate=>candidate.frames.includes(next));
      setSelectedFrame(next);if(owner>=0)setSelectedCall(owner);
    },
  });
