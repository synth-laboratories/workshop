...
function StandardTraceV5Viewer({ lane }: { lane: Lane }) {
  ...
  return <section aria-label="Trace V5 viewer" data-testid={`trace-v5-viewer-${lane.name}`} style={{ marginTop: 4, paddingTop: 12, borderTop: "1px solid var(--sv-border)" }}>
    ...
    <TraceV5EventList stateKey={`annotated.trace.${lane.name}`} items={projection.items} defaultView="focus" emptyToolText="No structured tool calls were retained for this rollout." />
...
function LaneCard({ lane, showHistory, streamBase }: { lane: Lane; showHistory: boolean; streamBase: URL | null }) {
  const [tab, setTab] = useVisualState<DetailTab>(`annotated.lane.${lane.name}.tab`, "rollout");
...
export function Shell(props: ShellProps) {
  const stream = useMemo(()=>retainedStreamInput(props.stream),[props.stream]);
  ...
  const [globalCursor, setGlobalCursor] = useVisualState<number | null>("annotated.globalCursor", null, {"type":"number"});
  const [playing, setPlaying] = useVisualState("annotated.playing", false);
  const [speed, setSpeed] = useVisualState("annotated.speed", 1);
  const [feed, setFeed] = useVisualState<Feed>("annotated.feed", "annotations");
  const [showHistory, setShowHistory] = useVisualState("annotated.showHistory", false);
  const [selectedRollout, setSelectedRollout] = useVisualState<string | null>("annotated.selectedRollout", null);
  ...
  const streamBase = typeof stream.sse_url === "string" && stream.sse_url ? new URL(stream.sse_url, window.location.href) : null;
  const selectedLane = lanes.find((lane) => lane.name === selectedRollout);

  useVisualPlaybackDriver({
    clock:"annotated.timeline", playingControl:"annotated.playing",
    playing:playing && timeline.length > 0, intervalMs:Math.max(40,360/speed),
    project:values=>{
      const next=Math.min(Number(values["annotated.globalCursor"] ?? -1)+1,timeline.length-1);
      return {"annotated.globalCursor":next,"annotated.playing":next<timeline.length-1};
    },
    fallback:()=>{
      const next=Math.min((globalCursor ?? -1)+1,timeline.length-1);
      setGlobalCursor(next); if(next>=timeline.length-1)setPlaying(false);
    },
  });
