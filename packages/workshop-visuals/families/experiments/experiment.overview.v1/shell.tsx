...
function OverviewStrip({ status, arms, model, progress }: { status: string; arms: Arm[]; model?: unknown; progress?: Progress }) {
  ...
  return <section className="sv-section" aria-label="Experiment summary" style={{ padding: "10px 12px", background: "#fffaf4", borderColor: "#e2d2c2" }}><dl style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(90px,1fr))", gap: 10, margin: 0 }}>{items.map(([label, value]) => <div key={String(label)}><dt style={{ color: "var(--sv-text-faint)", fontSize: 9.5, letterSpacing: ".06em", textTransform: "uppercase" }}>{label}</dt><dd style={{ margin: "3px 0 0", overflow: "hidden", fontFamily: "var(--sv-mono)", fontSize: 10, textOverflow: "ellipsis", textTransform: label === "Status" ? "capitalize" : undefined, whiteSpace: "nowrap" }}>{String(value)}</dd></div>)}</dl></section>;
}
...
function Disclosure({ title, summary, children, defaultOpen = false }: { title: string; summary?: string; children: ReactNode; defaultOpen?: boolean }) {
  return <details className="sv-section" style={{ padding: 0 }} open={defaultOpen || undefined}>
    <summary style={{ display: "flex", alignItems: "center", gap: 7, padding: "6px 8px", cursor: "pointer", listStyle: "none" }}><strong style={{ fontSize: 10 }}>{title}</strong>{summary ? <span style={{ marginLeft: "auto", color: "var(--sv-text-faint)", fontSize: 9.5 }}>{summary}</span> : null}<span aria-hidden style={{ color: "#2563a7", fontSize: 9.5 }}>Show</span></summary>
...
/**
 * A measured quantity -- a mean, a score, a reward -- at a fixed precision.
 *
 * `String(value)` printed a mean reward of 1.0 as `1`, so the same number read
 * `1` in the metric cards, `1` in the variants row, and `1.000` in the
 * assessment prose on one screen, while every sibling surface writes `1.00`.
 * At a glance `1` is a count; the decimals are what mark it as a measurement.
 *
 * Counts do not come here: `OverviewStrip` renders `Runs` and `Progress` with
 * its own formatter, and this function's only callers are metric values and
 * variant scores.
 */
...
function ProgressPanel({ progress, status }: { progress?: Progress; status?: string }) {
  ...
  return <section className="sv-section" aria-label="Experiment progress">
    ...
    <dl style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(90px,1fr))", gap: 10, margin: "13px 0 0" }}>
      {/*
        A finished run has no estimated time of arrival, so `ETA —` reports it
        as a measurement that went missing rather than as one that stopped
        applying. Drop the field once the run is terminal: the other three are
        still meaningful about a run that has ended, and a dash there is honest.
      */}
      {[["Elapsed", progress?.elapsed], ...(terminal ? [] : [["ETA", progress?.eta] as const]), ["Usage", progress?.usage], ["Cost", progress?.cost]].map(([label, value]) => <div key={label}><dt style={{ fontSize: 9, color: "var(--sv-text-faint)", textTransform: "uppercase" }}>{label}</dt><dd style={{ margin: "3px 0 0", fontFamily: "var(--sv-mono)", fontSize: 11 }}>{value ?? MISSING}</dd></div>)}
    </dl>
  </section>;
}
...
function Metrics({ metrics }: { metrics: Metric[] }) {
  ...
  return <section className="sv-section" aria-label="Experiment results"><div className="sv-metrics">{metrics.map((metric, index) => <div className="sv-metric" key={`${metric.label}-${index}`}><span>{metric.label}</span><strong style={{ color: metric.tone === "positive" ? "#18794e" : metric.tone === "negative" ? "#b42318" : undefined }}>{measured(metric.value)}</strong>{metric.detail ? <small>{metric.detail}</small> : null}</div>)}</div></section>;
}
...
function ContextGrid({ title, records }: { title: string; records: Array<{ label: string; data?: ContextRecord }> }) {
  ...
  return <section className="sv-section" aria-label={title}>{groups.map((group) => <div key={group.label} style={{ marginBottom: 8 }}><div className="sv-section-head"><h3>{group.label}</h3><span>{group.entries.length} fields</span></div><dl style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: "5px 10px", margin: 0 }}>{group.entries.map(([key, value]) => <div key={key} style={{ minWidth: 0 }}><dt style={{ color: "var(--sv-text-faint)", fontSize: 9.5, textTransform: "uppercase" }}>{key.replace(/([a-z])([A-Z])/g, "$1 $2")}</dt><dd style={{ margin: "2px 0 0", overflow: "hidden", fontFamily: "var(--sv-mono)", fontSize: 9, textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={String(value)}>{String(value)}</dd></div>)}</dl></div>)}</section>;
}
