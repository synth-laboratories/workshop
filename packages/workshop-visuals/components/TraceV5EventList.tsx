...
export function TraceV5EventList({
  ...
  const [view, setView] = useVisualState<"focus" | "full">(`${stateKey}.view`,defaultView,{options:["focus","full"]});
  const [query, setQuery] = useVisualState(`${stateKey}.query`,"");
  const [expanded, setExpanded] = useVisualState<string[]>(`${stateKey}.expanded`,[],{items:{type:"string"},maxItems:1000});
              ...
              <div style={{ border: `1px solid ${selectedItemId === item.id ? "var(--sv-accent)" : "var(--sv-border)"}`, borderRadius: 9, overflow: "hidden", background: "var(--sv-surface)" }}>
                ...
                <div style={{ padding: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: isLong || item.candidateId ? 7 : 0 }}>
                    ...
                    {isLong ? <button type="button" className="sv-btn" aria-expanded={open} onClick={() => setExpanded((current) => current.includes(item.id)?current.filter(id=>id!==item.id):[...current,item.id])} style={{ fontSize: 10 }}>{open ? "Close" : item.openLabel ?? "Show all"}</button> : null}
