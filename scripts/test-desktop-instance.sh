...
[[ "$(printf '%s' "$default_instance" | jq -r .releaseSlug)" == "v09" ]]
[[ "$(printf '%s' "$default_instance" | jq -r .instanceRoot)" == "$TEST_ROOT/instances/v09/$(printf '%s' "$default_instance" | jq -r .name)" ]]
...
alpha_manifest="$TEST_ROOT/instances/v09/alpha/instance.json"
...
' >/dev/null
alpha_env="$TEST_ROOT/instances/v09/alpha/data/.env"
...
if SYNTH_DESKTOP_RELEASE_LINE=v0.8 "$ROOT/scripts/desktop-instance.sh" print alpha >/dev/null 2>&1; then
  echo "non-v0.9 release line was accepted by the v0.9 launcher" >&2
  exit 1
fi
...
rg -q 'com.synth.desktop.v09.candidate' "$ROOT/apps/synth_desktop/src-tauri/tauri.candidate.conf.json"
...
lock_file="$TEST_ROOT/instances/v09/alpha/operation.lock"
...
descriptor="$TEST_ROOT/instances/v09/alpha/generated/descriptor-preview.app/Contents/Resources/instance.json"
...
jq -e '
  ...
  (.instance_root | endswith("/instances/v09/alpha")) and
  (.config_path | endswith("/instances/v09/alpha/data/config.toml")) and
  (.data_root | endswith("/instances/v09/alpha/data")) and
  .bundle_id == "com.synth.desktop.v09.dev.alpha" and
  .release_line == "v0.9" and
...
' "$TEST_ROOT/instances/v09/alpha/instance.json" >/dev/null
